...
export class AnthropicHandler extends BaseProvider implements SingleCompletionHandler {
	...
	async *createMessage(
		...
		// 在最后一个 tool 上设置 cache_control，利用 Anthropic 的渲染顺序（tools → system → messages）
		// 使 tools 定义成为缓存前缀的一部分，提高缓存命中率
									...
									return {
										...
										content:
											typeof message.content === "string"
												...
												: (() => {
														// 优先将 cache_control 设在 <environment_details> block 上（稳定内容），
														// 而不是最后一个 block（可能是易变的 <volatile_context>）。
														// 这样易变内容（时间戳、cost）在断点之后，不会破坏缓存前缀。
														const envBlockIndex = message.content.findIndex(
															(c) =>
																c.type === "text" &&
																typeof c.text === "string" &&
																c.text.trim().startsWith("<environment_details>"),
														)
														if (envBlockIndex >= 0) {
															return message.content.map((content, contentIndex) =>
																contentIndex === envBlockIndex
																	? { ...content, cache_control: cacheControl }
																	: content,
															)
														}
														// Fallback: 没有 environment_details 时，设在最后一个 block 上
														return message.content.map((content, contentIndex) =>
