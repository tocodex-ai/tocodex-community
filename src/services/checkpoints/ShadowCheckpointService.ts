...
export abstract class ShadowCheckpointService extends EventEmitter {
	...
	constructor(taskId: string, checkpointsDir: string, workspaceDir: string, log: (message: string) => void) {
		...
		// 受保护目录检查：只记录警告，不抛异常。
		// 上层调用方会在 catch 中禁用检查点，不影响扩展正常运行。
		...
		if (protectedPaths.includes(workspaceDir)) {
			log(
				`[ShadowCheckpointService] workspace is a protected directory (${workspaceDir}), checkpoints will be disabled`,
			)
			throw new Error(`Cannot use checkpoints in protected directory: ${workspaceDir}`)
		}
	...
	public async initShadowGit(onInit?: () => Promise<void>) {
		...
		// 嵌套 Git 检查改为非阻塞：检测到嵌套仓库只记录警告，不再禁用检查点。
		// 嵌套仓库的文件会被影子 Git 一起追踪，不影响检查点核心功能。
		try {
			const nestedGitPath = await this.getNestedGitRepository()
			if (nestedGitPath) {
				const relativePath = path.relative(this.workspaceDir, nestedGitPath)
				this.log(
					`[${this.constructor.name}#initShadowGit] nested git repository detected at: ${relativePath}, continuing anyway`,
				)
			}
		} catch (error) {
			// 嵌套检测失败不阻塞初始化
			this.log(
				`[${this.constructor.name}#initShadowGit] nested git check failed, continuing: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
		...
		if (await fileExistsAtPath(this.dotGitDir)) {
			...
			// 工作区路径变更时，自动更新 core.worktree 而不是报错禁用
			if (!arePathsEqual(worktreeTrimmed, this.workspaceDir)) {
				this.log(
					`[${this.constructor.name}#initShadowGit] workspace path changed from ${worktreeTrimmed} to ${this.workspaceDir}, updating config`,
				)
				await git.addConfig("core.worktree", this.workspaceDir)
				this.shadowGitConfigWorktree = this.workspaceDir
			}

			// 确保 gpgSign 配置存在（旧的影子仓库可能缺少此配置）
			try {
				const gpgSign = await git.getConfig("commit.gpgSign")
				if (gpgSign.value !== "false") {
			...
			await git.addConfig("core.worktree", this.workspaceDir)
			await git.addConfig("commit.gpgSign", "false")
			await git.addConfig("user.name", "ToCodex")
	...
	private async getNestedGitRepository(): Promise<string | null> {
		try {
			// 查找非根级的 .git/HEAD 文件。
			// 不使用 --follow 避免符号链接循环导致卡死。
			const args = ["--files", "--hidden", "-g", "**/.git/HEAD", this.workspaceDir]

			const gitPaths = await Promise.race([
				executeRipgrep({ args, workspacePath: this.workspaceDir }),
				// 嵌套检测最多等 3 秒，超时则跳过
				new Promise<never>((_, reject) =>
					setTimeout(() => reject(new Error("nested git check timed out")), 3000),
				),
			])
