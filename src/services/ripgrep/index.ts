 ...
 * Falls back to system PATH if not found in the VSCode bundle (e.g. in Electron desktop shell).
 */
export async function getBinPath(vscodeAppRoot: string): Promise<string | undefined> {
	...
	// Fallback: try to find ripgrep in system PATH (desktop shell / standalone environments)
	try {
		const cmd = isWindows ? "where" : "which"
		const result = childProcess.execSync(`${cmd} rg`, { encoding: "utf-8", timeout: 5000 }).trim()
		if (result) {
			const firstLine = result.split(/\r?\n/)[0]
			if (await fileExistsAtPath(firstLine)) {
				return firstLine
			}
		}
	} catch {
		// rg not in PATH either
	}
