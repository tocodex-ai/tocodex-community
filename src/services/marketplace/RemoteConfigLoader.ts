...
export class RemoteConfigLoader {
	...
	constructor() {
		this.apiBaseUrl = process.env.TOCODEX_MARKETPLACE_URL || "https://ruteapi.com/skills"
	}
	...
	private async fetchModes(): Promise<MarketplaceItem[]> {
		...
		const data = await this.fetchWithRetry<string>(`${this.apiBaseUrl}/modes.yaml`)
	...
	private async fetchMcps(): Promise<MarketplaceItem[]> {
		...
		const data = await this.fetchWithRetry<string>(`${this.apiBaseUrl}/mcps.yaml`)
