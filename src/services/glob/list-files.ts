...
export async function listFiles(dirPath: string, recursive: boolean, limit: number): Promise<[string[], boolean]> {
	...
	// Get ripgrep path — may be undefined in desktop shell environments
	let rgPath: string | undefined
	try {
		rgPath = await getRipgrepPath()
	} catch {
		// ripgrep not available — will fall back to Node.js traversal below
	}

	if (!rgPath) {
		// Fallback: pure Node.js file traversal (no ripgrep dependency)
		return listFilesWithNodeFallback(dirPath, recursive, limit)
	}
...
/**
 * Fallback file listing using pure Node.js fs operations when ripgrep is unavailable.
 * Respects .gitignore via the ignore package and skips common non-essential directories.
 */
async function listFilesWithNodeFallback(
	dirPath: string,
	recursive: boolean,
	limit: number,
	...
	async function walk(dir: string, depth: number): Promise<void> {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				if (DIRS_TO_IGNORE.includes(entry.name) || entry.name.startsWith(".")) {
					continue
				}
				// Check .gitignore
				try {
				...
				if (results.length < limit) {
					results.push(fullPath.endsWith("/") ? fullPath : `${fullPath}/`)
				} else {
					limitReached = true
					return
				}
...
function shouldIncludeDirectory(dirName: string, fullDirPath: string, context: ScanContext): boolean {
	...
	// This preserves the ability to explicitly target hidden directories like .tocodex-memory
