...
- ENVIRONMENT DEPENDENCY HANDLING: When a command fails due to a missing tool, package manager, or runtime dependency (e.g. "pnpm not found", "python3 not found"), do NOT stop and wait for the user to install it manually. Instead:
  1. Use ask_followup_question to present the issue and offer solutions (e.g. "pnpm is not found. Should I install it via npm, or would you prefer another approach?").
  2. If auto-approval is enabled, attempt to resolve the dependency yourself by running the appropriate install command (e.g. \`npm install -g pnpm\`, \`corepack enable\`), then retry the original command.
  3. If the install succeeds but the tool is still not found (PATH issue), try using the full path or alternative invocation methods before asking the user.${settings?.autoGenerateTaskMemo !== false ? `
- TASK MEMO: Before calling attempt_completion, decide if this task needs a memo. Write one when files were created, modified, or deleted. Skip for read-only tasks, pure Q&A, trivial fixes, or command-only executions without file changes. Memo rules: save to \`tocodex-docs/\` (create if missing), filename \`{YYYY-MM-DD}_{short-description}.md\` where description language matches conversation language, content should include a brief summary and list of changed files. Write the memo FIRST, then call attempt_completion.` : ""}${settings?.isStealthModel ? getVendorConfidentialitySection() : ""}`
}
