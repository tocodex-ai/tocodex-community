...
import { loadPlansSummary } from "../../../services/plan-storage/PlanStorage"
...
export async function loadRuleFiles(
	...
	options?: { autoGenerateTaskMemo?: boolean },
): Promise<string> {
	const rules: string[] = []
	...
	// Check for .tocodex/rules/ directories in order (global, project-local, and optionally subfolders)
	for (const rooDir of rooDirectories) {
		const rulesDir = path.join(rooDir, "rules")
		if (await directoryExists(rulesDir)) {
			let files = await readTextFilesFromDirectory(rulesDir)
			// When autoGenerateTaskMemo is disabled, filter out task-memo rule files
			if (options?.autoGenerateTaskMemo === false) {
				files = files.filter(
					(f) => !f.filename.toLowerCase().includes("task-memo"),
				)
			}
			if (files.length > 0) {
	...
	// If we found rules in .tocodex/rules/ directories, return them
	if (rules.length > 0) {
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		// Check for .tocodex/rules-${mode}/ directories in order (global, project-local, and optionally subfolders)
		for (const rooDir of rooDirectories) {
		...
		// If we found mode-specific rules in .tocodex/rules-${mode}/ directories, use them
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".tocodex", `rules-${mode}`))) {
	...
	const genericRuleContent = await loadRuleFiles(cwd, enableSubfolderRules, {
		autoGenerateTaskMemo: options.settings?.autoGenerateTaskMemo,
	})
	...
	// 加载已保存的计划（仅在 architect/code 模式且设置启用时）
	const PLAN_ENABLED_MODES = ["architect", "code"]
	if (options.settings?.loadRecentPlans !== false && PLAN_ENABLED_MODES.includes(mode)) {
		const plansSummary = await loadPlansSummary(cwd)
		if (plansSummary.trim()) {
			sections.push(plansSummary.trim())
		}
	}
