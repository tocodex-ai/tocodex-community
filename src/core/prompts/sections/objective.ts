/**
 * 返回 OBJECTIVE 段落。
 * 首轮返回完整方法论指导，后续轮次返回精简版以节省 token。
 *
 * @param isFirstTurn - 是否为对话的首轮（默认 true，保持向后兼容）
 */
export function getObjectiveSection(isFirstTurn: boolean = true): string {
	if (!isFirstTurn) {
		return `====

OBJECTIVE

Accomplish the user's task iteratively: break it into steps, use tools as needed, and call attempt_completion when done. Do not end with questions or offers for further assistance.`
	}
