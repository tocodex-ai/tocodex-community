		...
		if (rooIgnoreController) {
			...
			for (const filePath of sorted) {
				...
				if (isIgnored) {
					...
					// 源头过滤：只对 cwd 子树内的文件检查保护，跨盘符 / cwd 外路径直接跳过。
					// 防御深度——即便上层（如调用方传入的 absolutePath 本身就在 cwd 外）出现，
					// 也避免 isWriteProtected 触发 ignore 库的 RangeError 异常路径。
					const relForProtect = path.relative(absolutePath, absoluteFilePath)
					const inSubtree =
						!relForProtect.startsWith("..") &&
						!path.isAbsolute(relForProtect) &&
						!relForProtect.startsWith("/")
					const isWriteProtected =
						inSubtree && (rooProtectedController?.isWriteProtected(absoluteFilePath) || false)
					if (isWriteProtected) {
