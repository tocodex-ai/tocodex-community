...
export async function generateErrorDiagnostics(params: GenerateDiagnosticsParams): Promise<GenerateDiagnosticsResult> {
	...
	try {
		...
		const headerComment =
			"// Please share this file with ToCodex Support (support@tocodex.com) to diagnose the issue faster\n" +
		...
		const tempFileName = `tocodex-diagnostics-${taskId.slice(0, 8)}-${timestamp}.json`
