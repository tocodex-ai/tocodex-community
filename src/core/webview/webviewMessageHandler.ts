...
import { getOpenAiModels, getOpenAiModelsWithMetadata } from "../../api/providers/openai"
...
import { getDefaultFallbackModels } from "../../api/providers/fetchers/roo"
	...
	switch (message.type as string) {
		case "webviewDidLaunch":
			...
			try {
				...
				console.error("[webviewDidLaunch] Error loading custom modes:", error)
			}

			provider.postStateToWebview().catch((error) => {
				console.error("[webviewDidLaunch] Error posting state to webview:", error)
			})
			...
			// 在 webview 启动时立即推送 roo 模型（缓存或 fallback）给 UI
			// 这样 UI 不需要等待 requestRouterModels 的 API 调用就能立即显示模型列表
			// 后续 useRouterModels hook 的 requestRouterModels 调用会覆盖更新
			;(async () => {
				try {
					// 如果刷新已成功，用最新缓存；否则用 fallback
					// 防止 fallback 覆盖刷新后的新列表（竞争条件修复）
					if (hasRefreshSucceeded("roo")) {
						// 刷新已完成，内存缓存是最新数据，直接用缓存
						const cachedModels = getModelsFromCache("roo")
						if (cachedModels && Object.keys(cachedModels).length > 0) {
							provider.postMessageToWebview({
								type: "routerModels",
								routerModels: { roo: cachedModels } as any,
								values: { provider: "roo" },
							})
						}
					} else {
						// 刷新尚未完成，推送缓存或 fallback
						let rooModels = getModelsFromCache("roo")
						if (!rooModels || Object.keys(rooModels).length === 0) {
							rooModels = getDefaultFallbackModels()
						}
						provider.postMessageToWebview({
							type: "routerModels",
							routerModels: { roo: rooModels } as any,
							values: { provider: "roo" },
						})
					}
				} catch (e) {
		...
		case "pasteImageFromClipboard": {
			try {
				...
				if (platform === "win32") {
					exec("snippingtool /clip", (error: any) => {
						if (error) {
					...
					exec("gnome-screenshot -a -c", () => {})
				}
			} catch (error) {
				provider.log(`[pasteImageFromClipboard] Error: ${error}`)
			}
			break
		}
		...
		case "condenseTaskContextRequest":
			...
			break
		case "forceTruncateContext":
			provider.forceTruncateTaskContext(message.text!)
			break
		...
		case "requestRouterModels":
			...
			if (shouldRefresh && providerFilter && modelFetchPromises.length > 0) {
				...
				// 同步刷新时清除该 provider 的自定义模型
				const customKey = `customModels-${providerFilter}`
				provider.log(`[requestRouterModels] Clearing custom models: ${customKey}`)
				await provider.context.globalState.update(customKey, undefined)
			} else if (shouldRefresh && !providerFilter) {
				// 无特定 provider 时清除所有自定义模型
				for (const key of ["roo", "openrouter", "requesty", "unbound", "vercel-ai-gateway"]) {
					await provider.context.globalState.update(`customModels-${key}`, undefined)
				}
			}
			...
			results.forEach((result, index) => {
				...
				if (result.status === "fulfilled") {
					let resolvedModels = result.value.models

					// 防止竞态：如果 roo 的后台刷新已完成，且 getModels 返回的数据
					// 比缓存中的数据更少（说明是 fallback 或旧数据），用刷新后的缓存
					if (routerName === "roo" && hasRefreshSucceeded("roo")) {
						const cachedModels = getModelsFromCache("roo")
						if (cachedModels && Object.keys(cachedModels).length > Object.keys(resolvedModels).length) {
							resolvedModels = cachedModels
						}
					}

					routerModels[routerName] = resolvedModels
					...
					// 防止竞态：如果 roo 已刷新成功，不用空对象覆盖缓存
					if (routerName === "roo" && hasRefreshSucceeded("roo")) {
						const cachedModels = getModelsFromCache("roo")
						routerModels[routerName] =
							cachedModels && Object.keys(cachedModels).length > 0 ? cachedModels : {}
					} else {
			...
			// 合并自定义模型到模型列表
			for (const routerName of Object.keys(routerModels) as RouterName[]) {
				const customModelsKey = `customModels-${routerName}`
				const customModelIds: string[] = provider.context.globalState.get(customModelsKey) || []
				for (const modelId of customModelIds) {
					if (!routerModels[routerName][modelId]) {
						routerModels[routerName][modelId] = {
							maxTokens: 4096,
		...
		case "requestOpenAiModels":
			// 只要有 baseUrl 就尝试请求，apiKey 改为可选（兼容本地 / 自托管无认证中转站）
			if (message?.values?.baseUrl) {
				const baseUrl = message?.values?.baseUrl as string
				const apiKey = message?.values?.apiKey as string | undefined
				const openAiHeaders = message?.values?.openAiHeaders as Record<string, string> | undefined
				const openAiModelInfos = await getOpenAiModelsWithMetadata(baseUrl, apiKey, openAiHeaders)
				const openAiModels = openAiModelInfos.map((m) => m.id)

				provider.postMessageToWebview({
					type: "openAiModels",
					openAiModels,
					values: { openAiModelInfos },
				})
			}
		...
		case "saveImage": {
			...
			if (imagePath || dataUri) {
				// Determine file extension for the default filename
				let ext = "png"
				if (imagePath) {
					ext = path.extname(imagePath).toLowerCase().replace(".", "") || "png"
				} else if (dataUri) {
				...
				const defaultFileName = `img_${Date.now()}.${ext}`
				...
				const savedUri = await saveImage(dataUri || "", defaultUri, imagePath)
		...
		case "openProjectMcpSettings": {
			...
			const rooDir = path.join(workspaceFolder, ".tocodex")
		...
		case "autoApprovalEnabled":
			...
			await provider.postStateToWebview()
			break
		case "maxParallelChildren":
			await updateGlobalState("maxParallelChildren", message.value ?? 10)
			await provider.postStateToWebview()
			break
		case "getMaxParallelChildren":
			const savedMaxParallel = getGlobalState("maxParallelChildren") ?? 10
			await provider.postMessageToWebview({
				type: "maxParallelChildrenValue",
				value: savedMaxParallel,
			})
			break
		case "enhancePrompt":
			if (message.text) {
		...
		case "deleteCustomMode":
			if (message.slug) {
				...
				if (scope === "project") {
					...
					if (workspacePath) {
						rulesFolderPath = path.join(workspacePath, ".tocodex", `rules-${message.slug}`)
					} else {
						rulesFolderPath = path.join(".tocodex", `rules-${message.slug}`)
					}
				} else {
					// Global scope - use OS home directory
					const homeDir = os.homedir()
					rulesFolderPath = path.join(homeDir, ".tocodex", `rules-${message.slug}`)
				}
		...
		case "rooCloudManualApiKey": {
			// 阶段 8.10 桌面壳诊断：用户报告"还是一样"，前两轮修复未解决问题。
			// 改用文件级诊断，把每步实际状态写到 exe 同目录 tocodex-diag-key.txt，
			// 运行后用户把这个文件提供给我，根据真实数据定位根因。
			const diagWrite = (line: string) => {
				try {
					const fs = require("node:fs") as typeof import("node:fs")
					const path = require("node:path") as typeof import("node:path")
					// 优先写到 exe 同目录（Electron 打包后），其次写到 cwd
					const candidates = [
						path.join(path.dirname(process.execPath), "tocodex-diag-key.txt"),
						path.join(process.cwd(), "tocodex-diag-key.txt"),
					]
			...
			try {
				diagWrite(`[manualApiKey] handler entered; hasText=${!!message.text}`)
				provider.log(`[Diag#manualApiKey] received manual key bind request; hasText=${!!message.text}`)
				if (!message.text) {
					vscode.window.showErrorMessage("API Key is empty")
					break
				}

				const apiKey = message.text.trim()

				// ⚠️ 后端防御性校验：HTTP Authorization 头部只接受 ISO-8859-1（ASCII），
				// 如果 API Key 含中文/emoji，fetch 会抛 "Cannot convert argument to a ByteString" 错误。
				// 前端 WelcomeViewProvider 已做校验，这里是第二道防线（防止 DevTools 绕过）。
				if (!/^[\x21-\x7E]+$/.test(apiKey)) {
					provider.log(`[rooCloudManualApiKey] 拒绝非 ASCII Key：${JSON.stringify(apiKey).slice(0, 50)}`)
					vscode.window.showErrorMessage(
						"API Key 包含中文或不可见字符，无法使用。请粘贴正确的 sk- 开头的官方 Key。",
					)
					break
				}
				if (apiKey.length < 16) {
					provider.log(`[rooCloudManualApiKey] 拒绝长度不足的 Key：length=${apiKey.length}`)
					vscode.window.showErrorMessage("API Key 长度不足，请粘贴完整的官方 Key。")
					break
				}

				diagWrite(`[manualApiKey] validated; length=${apiKey.length}`)
				provider.log(
					`[Diag#manualApiKey] validated key length=${apiKey.length}; before store provider=${provider.contextProxy.getGlobalState("apiProvider")}, model=${provider.contextProxy.getGlobalState("apiModelId")}, profile=${provider.contextProxy.getGlobalState("currentApiConfigName")}`,
				)

				// 直接存储 API Key 并同步到 ContextProxy secret cache。
				// 不能只写 provider.context.secrets.store("tocodex-api-key")：
				// getState()/当前 Task handler 使用的是 ContextProxy 中的 rooApiKey，
				// 只写外部 secret 会导致绑定后图标/设置/发消息看到的状态不一致。
				await provider.context.secrets.store("tocodex-api-key", apiKey)
				await provider.contextProxy.storeSecret("rooApiKey", apiKey)
				diagWrite(
					`[manualApiKey] secrets stored; rooApiKey from contextProxy=${provider.contextProxy.getSecret("rooApiKey") ? "present" : "missing"}`,
				)
				provider.log(`[Diag#manualApiKey] secret stored; changing auth state if WebAuthService`)

				// 使用鸭子类型检查代替 instanceof，因为在 Electron 桌面壳中
				// 插件 bundle 通过 require(bundlePath) 加载，与 vscode-shim 可能使用
				// 不同的模块实例，导致 instanceof WebAuthService 跨模块上下文失败。
				const authService = CloudService.instance.authService
				const authServiceCtor = authService ? (authService as any).constructor?.name : "null"
				const hasSetManual = authService && typeof (authService as any).setManualApiKey === "function"
				const hasChangeState = authService && typeof (authService as any).changeState === "function"
				diagWrite(
					`[manualApiKey] authService ctor=${authServiceCtor}, hasSetManualApiKey=${hasSetManual}, hasChangeState=${hasChangeState}`,
				)
				if (hasSetManual) {
					;(authService as any).setManualApiKey(apiKey)
					diagWrite(`[manualApiKey] called setManualApiKey`)
				} else if (hasChangeState) {
					// 兜底：旧版 WebAuthService 没有 setManualApiKey 时直接操作
					;(authService as any).apiKey = apiKey
					;(authService as any).changeState("active-session")
					diagWrite(`[manualApiKey] called changeState directly`)
				} else {
					diagWrite(`[manualApiKey] NO auth state change applied!`)
				}

				// 立即检查 isAuthenticated 是否变了
				try {
					const isAuth = CloudService.instance.isAuthenticated()
					const authState = (authService as any)?.state
					diagWrite(`[manualApiKey] post-change: isAuthenticated=${isAuth}, state=${authState}`)
				} catch (e) {
					diagWrite(
						`[manualApiKey] isAuthenticated check threw: ${e instanceof Error ? e.message : String(e)}`,
					)
				}

				provider.log(
					`[Diag#manualApiKey] after auth change provider=${provider.contextProxy.getGlobalState("apiProvider")}, model=${provider.contextProxy.getGlobalState("apiModelId")}`,
				)
				vscode.window.showInformationMessage("Successfully bound ToCodex API Key")

				// 绑定官方 key 时，清除第三方供应商设置，重置为官方 roo 配置
				// 同时更新 providerProfile 以触发 SettingsView 的 cachedState 刷新
				const existingState = await provider.getState()
				const existingModelId = existingState.apiConfiguration?.apiModelId
				const rooConfig: ProviderSettings = {
					apiProvider: "roo",
					rooApiKey: apiKey,
					apiModelId: existingModelId || "gpt-5.4",
				}
				const { currentApiConfigName } = existingState
				provider.log(
					`[Diag#manualApiKey] before upsertProviderProfile currentApiConfigName=${currentApiConfigName}`,
				)
				await provider.upsertProviderProfile(currentApiConfigName || "default", rooConfig)
				provider.log(
					`[Diag#manualApiKey] after upsert provider=${provider.contextProxy.getGlobalState("apiProvider")}, model=${provider.contextProxy.getGlobalState("apiModelId")}, profile=${provider.contextProxy.getGlobalState("currentApiConfigName")}`,
				)
				...
				try {
					...
					provider.log(`[ManualApiKey] Failed to refresh models: ${modelError}`)
				}

				await provider.postStateToWebview()
			} catch (error) {
				provider.log(`ManualApiKey failed: ${error}`)
				vscode.window.showErrorMessage(
					`Failed to bind API Key: ${error instanceof Error ? error.message : String(error)}`,
				)
			}
			break
		}
		case "getApiKey": {
			try {
				const apiKey = await provider.context.secrets.get("tocodex-api-key")
				if (apiKey) {
					await provider.postMessageToWebview({ type: "apiKeyValue", text: apiKey } as any)
				}
			} catch (error) {
				provider.log(`getApiKey failed: ${error}`)
			}
			break
		}
		case "addCustomModel": {
			try {
				const modelId = message.text?.trim()
				const providerName = message.values?.provider || "roo"
				if (!modelId) break

				// 从 globalState 获取自定义模型列表
				const customModelsKey = `customModels-${providerName}`
				const existingModels: string[] = provider.context.globalState.get(customModelsKey) || []

				if (!existingModels.includes(modelId)) {
					existingModels.push(modelId)
					await provider.context.globalState.update(customModelsKey, existingModels)
					provider.log(`[addCustomModel] Added custom model: ${modelId} for provider: ${providerName}`)
				}
			} catch (error) {
				provider.log(`[addCustomModel] Failed: ${error}`)
			}
			break
		}
		...
		case "createCommand": {
			try {
				...
				if (source === "global") {
					const globalConfigDir = path.join(os.homedir(), ".tocodex")
					...
					commandsDir = path.join(workspaceRoot, ".tocodex", "commands")
				}
		...
		case "rememberMemory": {
			try {
				...
				if (!text) {
					vscode.window.showWarningMessage("记忆内容不能为空")
					break
				}

				const cwd = getCurrentCwd()
				const { MemoryManager } = await import("../../services/memory/MemoryManager")
				const memoryManager = new MemoryManager({ scope: "project", cwd })
				const added = await memoryManager.appendMemory(text)

				if (added) {
					vscode.window.showInformationMessage(
						`已保存项目记忆: ${text.length > 50 ? text.slice(0, 50) + "..." : text}`,
					)
				} else {
					vscode.window.showInformationMessage("该记忆已存在，未重复保存")
				}
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error saving memory: ${errorMessage}`)
				vscode.window.showErrorMessage(`保存记忆失败: ${errorMessage}`)
			}
			break
		}
		...
		case "forgetMemory": {
			try {
				...
				vscode.window.showInformationMessage("已清空当前项目的所有记忆")
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error clearing memory: ${errorMessage}`)
				vscode.window.showErrorMessage(`清空记忆失败: ${errorMessage}`)
			}
			break
		}

		case "rememberGlobalMemory": {
			try {
				const text = message.text?.trim()
				if (!text) {
					vscode.window.showWarningMessage("全局记忆内容不能为空")
					break
				}

				const globalStoragePath = provider.contextProxy.globalStorageUri.fsPath
				const { MemoryManager } = await import("../../services/memory/MemoryManager")
				const memoryManager = new MemoryManager({ scope: "global", globalStoragePath })
				const added = await memoryManager.appendMemory(text, "preference")

				if (added) {
					vscode.window.showInformationMessage(
						`已保存全局记忆: ${text.length > 50 ? text.slice(0, 50) + "..." : text}`,
					)
				} else {
					vscode.window.showInformationMessage("该全局记忆已存在，未重复保存")
				}
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error saving global memory: ${errorMessage}`)
				vscode.window.showErrorMessage(`保存全局记忆失败: ${errorMessage}`)
			}
			break
		}

		case "forgetGlobalMemory": {
			try {
				const globalStoragePath = provider.contextProxy.globalStorageUri.fsPath
				const { MemoryManager } = await import("../../services/memory/MemoryManager")
				const memoryManager = new MemoryManager({ scope: "global", globalStoragePath })
				...
				vscode.window.showInformationMessage("已清空所有全局记忆")
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error clearing global memory: ${errorMessage}`)
				vscode.window.showErrorMessage(`清空全局记忆失败: ${errorMessage}`)
			}
			break
		}

		case "loadHooksConfig": {
			try {
				const cwd = getCurrentCwd()
				const configPath = path.join(cwd, ".tocodex", "hooks.json")
				const raw = await fs.readFile(configPath, "utf-8")
				const hooksConfig = JSON.parse(raw)
				provider.postMessageToWebview({ type: "hooksConfigLoaded", hooksConfig })
			} catch {
				// 文件不存在或解析失败，返回空配置
				provider.postMessageToWebview({ type: "hooksConfigLoaded", hooksConfig: {} })
			}
		...
		case "saveHooksConfig": {
			try {
				const cwd = getCurrentCwd()
				const rooDir = path.join(cwd, ".tocodex")
				await fs.mkdir(rooDir, { recursive: true })
				const configPath = path.join(rooDir, "hooks.json")
				// 清理空条目（command 为空的 hook）
				const config = (message as any).hooksConfig ?? {}
				const cleaned: Record<string, any> = {}
				for (const [phase, entries] of Object.entries(config)) {
					if (Array.isArray(entries)) {
						const validEntries = entries.filter((e: any) => e.command?.trim())
						if (validEntries.length > 0) {
							cleaned[phase] = validEntries
						}
					}
				}
				await safeWriteJson(configPath, cleaned, { prettyPrint: true })
				provider.log("Hooks config saved successfully")
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error saving hooks config: ${errorMessage}`)
				vscode.window.showErrorMessage(`保存 Hooks 配置失败: ${errorMessage}`)
			}
			break
		}

		/**
		 * Task Progress Persistence (/resume)
		...
		case "resumeProgress": {
			try {
				...
				const activeProgress = await persistence.listActive()

				const results = activeProgress.map((p) => {
				...
				provider.postMessageToWebview({
					type: "resumeProgressResult",
					payload: results,
				})

				if (results.length === 0) {
					vscode.window.showInformationMessage("没有进行中的任务")
				} else {
					const items = results.map(
						(r) => `任务 ${r.taskId.slice(0, 8)}... — ${r.percent}% (${r.completed}/${r.total})`,
					)
					vscode.window.showInformationMessage(`进行中的任务:\n${items.join("\n")}`)
				}
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error listing active progress: ${errorMessage}`)
				// 进度文件损坏或缺失时返回空列表 (Requirements: 7.5)
				provider.postMessageToWebview({
					type: "resumeProgressResult",
					payload: [],
				})
		...
		case "loadScheduledTasks": {
			try {
				const store = (provider as any).getScheduledTaskStore?.()
				if (store) {
					// 每次打开"定时任务"面板时，重新从磁盘加载 .tocodex/scheduled-tasks.json
					// 这样用户/LLM 在外部直接修改 JSON 后无需重启即可生效
					try {
						await store.load()
					} catch (reloadErr) {
						const msg = reloadErr instanceof Error ? reloadErr.message : String(reloadErr)
						provider.log(`Error reloading scheduled tasks from disk: ${msg}`)
					}
				}
				const tasks = store ? store.getAll() : []
				provider.postMessageToWebview({
				...
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error loading scheduled tasks: ${errorMessage}`)
				provider.postMessageToWebview({
					type: "scheduledTasksLoaded",
					payload: [],
				})
		...
		case "createScheduledTask": {
			try {
				...
				if (!store) {
					provider.postMessageToWebview({
						type: "scheduledTaskOperationError",
						error: "Scheduled tasks subsystem not initialized",
					})
					break
				}
				const input = (message as any).payload
				const created = await store.create(input)
				provider.postMessageToWebview({ type: "scheduledTaskCreated", payload: created })
				// refresh list
				provider.postMessageToWebview({
					type: "scheduledTasksLoaded",
				...
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error creating scheduled task: ${errorMessage}`)
				provider.postMessageToWebview({
					type: "scheduledTaskOperationError",
					error: errorMessage,
				})
		...
		case "updateScheduledTask": {
			try {
				...
				const { id, patch } = (message as any).payload ?? {}
				const updated = await store.update(id, patch)
				provider.postMessageToWebview({ type: "scheduledTaskUpdated", payload: updated })
				provider.postMessageToWebview({
					type: "scheduledTasksLoaded",
					payload: store.getAll(),
				})
				...
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error updating scheduled task: ${errorMessage}`)
				provider.postMessageToWebview({
					type: "scheduledTaskOperationError",
					error: errorMessage,
				})
		...
		case "deleteScheduledTask": {
			try {
				const store = (provider as any).getScheduledTaskStore?.()
				if (!store) break
				const id = (message as any).payload?.id ?? (message as any).text
				await store.delete(id)
				provider.postMessageToWebview({ type: "scheduledTaskDeleted", payload: { id } })
				provider.postMessageToWebview({
					type: "scheduledTasksLoaded",
					payload: store.getAll(),
				})
				...
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error deleting scheduled task: ${errorMessage}`)
				provider.postMessageToWebview({
					type: "scheduledTaskOperationError",
					error: errorMessage,
				})
		...
		case "triggerScheduledTask": {
			try {
				const scheduler = (provider as any).getScheduledTaskScheduler?.()
				if (!scheduler) break
				const id = (message as any).payload?.id ?? (message as any).text
				const ok = scheduler.triggerNow(id)
				provider.postMessageToWebview({
					type: "scheduledTaskTriggered",
					payload: { id, success: ok },
				})
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error triggering scheduled task: ${errorMessage}`)
			}
			break
		}

		case "toggleScheduledTaskEnabled": {
			try {
				const store = (provider as any).getScheduledTaskStore?.()
				if (!store) break
				const { id, enabled } = (message as any).payload ?? {}
				...
				const errorMessage = error instanceof Error ? error.message : String(error)
				provider.log(`Error toggling scheduled task: ${errorMessage}`)
				provider.postMessageToWebview({
					type: "scheduledTaskOperationError",
					error: errorMessage,
				})
		...
		case "webviewHeartbeat" as any: {
			// 诊断：webview 主线程存活心跳。记录时间戳，由后台心跳监视器对比超时。
			;(provider as any)._lastWebviewHeartbeatAt = Date.now()
			;(provider as any)._lastWebviewHeartbeatBeats = (message as any)?.values?.beats
			break
		}

		case "webviewDiagnostic" as any: {
			// 诊断：webview 端 window.onerror / unhandledrejection 直接送回扩展进程
			const v = (message as any)?.values || {}
			provider.log(
				`[webviewDiagnostic] kind=${v.kind} msg=${(v.message ?? v.reason ?? "").toString().slice(0, 500)}`,
			)
			if (v.stack) {
				provider.log(`[webviewDiagnostic] stack:\n${String(v.stack).slice(0, 2000)}`)
			}
			if (v.filename) {
				provider.log(`[webviewDiagnostic] at ${v.filename}:${v.line}:${v.col}`)
			}
			break
		}
