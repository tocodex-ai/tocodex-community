...
export async function setTodoListForTask(cline?: Task, todos?: TodoItem[]) {
	...
	// 同步持久化到 .tocodex/progress/<task-id>.json (Requirements: 7.1)
	try {
		const persistence = new TaskProgressPersistence(cline.cwd)
		await persistence.save(cline.taskId, cline.todoList)
	} catch (error) {
		// 持久化失败不影响主流程，但记录日志便于诊断磁盘问题
		console.warn(`[UpdateTodoListTool] 持久化失败 (taskId=${cline.taskId}):`, error)
	}
}
