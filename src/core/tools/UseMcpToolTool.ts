	...
	private async executeToolAndProcessResult(
		...
		const toolResult = await task.providerRef.deref()?.getMcpHub()?.callTool(serverName, toolName, parsedArguments)
			.catch(async (error: Error) => {
				// 如果是 "Not connected" 错误，尝试自动重连后重试一次
				if (error.message?.includes("Not connected") || error.message?.includes("No connection found")) {
					const mcpHub = task.providerRef.deref()?.getMcpHub()
					if (mcpHub) {
						await mcpHub.refreshAllConnections()
						// 等待连接建立
						await new Promise(resolve => setTimeout(resolve, 3000))
						return mcpHub.callTool(serverName, toolName, parsedArguments)
					}
				}
				throw error
			})
