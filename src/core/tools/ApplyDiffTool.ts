...
export class ApplyDiffTool extends BaseTool<"apply_diff"> {
	...
	async execute(params: ApplyDiffParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			// 并行子任务文件锁检查
			if (task.isParallelChild) {
				const { fileLockRegistry } = await import("../../services/file-lock/FileLockRegistry")
				const { acquired, lockedBy } = fileLockRegistry.tryAcquire(task.taskId, relPath)
				if (!acquired) {
					task.consecutiveMistakeCount++
					task.recordToolError("apply_diff")
					pushToolResult(
						formatResponse.toolError(
							`File "${relPath}" is locked by another parallel sub-task (${lockedBy}). ` +
								`Parallel sub-tasks must not modify the same file. Skip this file or use a different filename.`,
						),
					)
					return
				}
			...
			if (!diffResult.success) {
				...
				if (currentCount >= 2) {
					...
					// 连续 2 次 apply_diff 失败后，建议模型降级到 write_to_file（Requirements: 5.2）
					formattedError += `\n\n<fallback_suggestion>\napply_diff has failed ${currentCount} consecutive times for this file. Consider using write_to_file instead to replace the entire file content. Use read_file first to get the current content, then apply your changes manually.\n</fallback_suggestion>`
				}
			...
			// apply_diff 后立即使缓存失效
			FileReadCache.getInstance().invalidate(relPath)
