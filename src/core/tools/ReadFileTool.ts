...
export class ReadFileTool extends BaseTool<"read_file"> {
		...
		try {
			...
			for (const fileResult of fileResults) {
				...
				try {
					...
					const fileCache = FileReadCache.getInstance()
					const mtime = stats.mtimeMs
					let fileContent = fileCache.get(relPath, mtime)
					if (fileContent === undefined) {
						const buffer = await fs.readFile(fullPath)
						fileContent = buffer.toString("utf-8")
						fileCache.set(relPath, fileContent, mtime)
					}
	...
	private buildAndPushResult(task: Task, fileResults: FileResult[], pushToolResult: PushToolResult): void {
		let finalResult = fileResults
			.filter((r) => r.nativeContent)
			.map((r) => r.nativeContent)
			.join("\n\n---\n\n")

		// 对文本结果应用智能摘要（Requirements: 18.1）
		const { content: summarizedResult, wasSummarized } = summarizeIfNeeded("read_file", finalResult)
		if (wasSummarized) {
			finalResult = summarizedResult
		}
	...
	private async executeLegacy(fileEntries: FileEntry[], task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		for (const entry of fileEntries) {
			...
			try {
				...
				// Read text file (with cache support)
				const fileCache = FileReadCache.getInstance()
				const mtime = stats.mtimeMs
				let rawContent = fileCache.get(relPath, mtime)
				if (rawContent === undefined) {
					rawContent = await fs.readFile(fullPath, "utf8")
					fileCache.set(relPath, rawContent, mtime)
				}
		...
		// Push combined results — 对文本结果应用智能摘要（Requirements: 18.1）
		const combined = results.join("\n\n---\n\n")
		const { content: summarizedCombined } = summarizeIfNeeded("read_file", combined)
		pushToolResult(summarizedCombined)
	}
}
