...
export interface EnvironmentDetailsResult {
	/** 稳定内容（文件列表、模式信息等），适合作为缓存前缀的一部分 */
	stableDetails: string
	/** 易变内容（时间戳、cost 等），每次请求都可能变化，放在 cache_control 断点之后 */
	volatileDetails: string
}

export async function getEnvironmentDetails(cline: Task, includeFileDetails?: boolean): Promise<string>
export async function getEnvironmentDetails(
	cline: Task,
	includeFileDetails: boolean,
	splitVolatile: true,
): Promise<EnvironmentDetailsResult>
export async function getEnvironmentDetails(
	cline: Task,
	includeFileDetails: boolean = false,
	splitVolatile?: boolean,
): Promise<string | EnvironmentDetailsResult> {
	let details = ""
	...
	// 易变内容（时间戳、cost）单独收集，避免破坏 prompt caching 前缀
	let volatileSection = ""
	...
	// splitVolatile 模式：将稳定内容和易变内容分开返回，
	// 让调用方可以将它们放入不同的 text block，使 cache_control 断点
	// 设在稳定内容上，易变内容放在断点之后，不破坏缓存前缀。
	if (splitVolatile) {
		const stableDetails = `<environment_details>\n${details.trim()}\n${reminderSection}\n</environment_details>`
		const volatileDetails = volatileSection.trim()
			? `<volatile_context>\n${volatileSection.trim()}\n</volatile_context>`
			: ""
		return { stableDetails, volatileDetails }
	}

	// 默认模式：保持向后兼容，所有内容合并到一个字符串中
	if (volatileSection) {
		details += volatileSection
	}
