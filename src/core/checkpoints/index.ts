	...
	try {
		...
		if (task.checkpointServiceInitializing) {
			...
			if (!task?.checkpointService) {
				...
				task.enableCheckpoints = false
				// 延迟清除警告，让 UI 显示一次后自动消失
				setTimeout(() => sendCheckpointInitWarn(task), 6000)
				return undefined
			} else {
		...
		// 使用 Promise.race 为整个初始化流程添加超时保护，
		// 防止 ripgrep 扫描嵌套 git 或 git add 大型项目时无限阻塞
		const initPromise = checkGitInstallation(task, service, log, provider)
		const timeoutPromise = new Promise<"timeout">((resolve) =>
			setTimeout(() => resolve("timeout"), checkpointTimeoutMs),
		)
		...
		if (result === "timeout") {
			log(
				`[Task#getCheckpointService] initialization timed out after ${task.checkpointTimeout}s, disabling checkpoints`,
			)
			sendCheckpointInitWarn(task, "INIT_TIMEOUT", task.checkpointTimeout)
			task.enableCheckpoints = false
			task.checkpointServiceInitializing = false
			// 延迟清除警告，让 UI 有时间显示一次提示后自动消失
			setTimeout(() => sendCheckpointInitWarn(task), 6000)
			return undefined
		}
