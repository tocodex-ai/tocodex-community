...
export class ContextProxy {
	...
	public async initialize() {
		...
		// Migration: Move the old default auto-condense threshold from 100% to 90%.
		await this.migrateDefaultAutoCondenseThreshold()

		this._isInitialized = true
	}

	/**
	 * Migrates the old default auto-condense threshold to the new 90% default.
	 *
	 * Existing users commonly have 100 persisted because it used to be the shipped default.
	 * Treat exactly 100 as the old default and migrate it once; other user-customized values are preserved.
	 */
	private async migrateDefaultAutoCondenseThreshold() {
		try {
			const migrationKey = "autoCondenseContextPercentDefault90Migrated"
			const migrationDone = this.originalContext.globalState.get<boolean>(migrationKey)
			const current = this.originalContext.globalState.get<number>("autoCondenseContextPercent")
			logger.info(
				`[ContextProxy] autoCondenseContextPercent migration check: done=${String(migrationDone)}, current=${String(current)}, cached=${String(this.stateCache.autoCondenseContextPercent)}`,
			)
			...
			if (current === undefined || current === 100) {
				await this.originalContext.globalState.update("autoCondenseContextPercent", 90)
				this.stateCache.autoCondenseContextPercent = 90
				logger.info("[ContextProxy] Migrated autoCondenseContextPercent default from 100 to 90")
			} else {
				logger.info(
					`[ContextProxy] Preserved custom autoCondenseContextPercent=${String(current)} during default migration`,
				)
			}

			await this.originalContext.globalState.update(migrationKey, true)
			logger.info(
				`[ContextProxy] autoCondenseContextPercent migration marked complete; stored=${String(this.originalContext.globalState.get<number>("autoCondenseContextPercent"))}`,
			)
		} catch (error) {
			logger.error(
				`Error during autoCondenseContextPercent default migration: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
	}
