...
import { buildLightModelHandler } from "../../api/light-model"
...
import { CostTracker, type TokenUsageForCost, type ModelPricing } from "./CostTracker"
import { TaskProgressSummarizer } from "./TaskProgressSummarizer"
import { HooksRunner } from "./HooksRunner"
import { restoreTodoListForTask } from "../tools/UpdateTodoListTool"
import { FileReadCache } from "../tools/FileReadCache"
import { FileContextTracker } from "../context-tracking/FileContextTracker"
import { RooIgnoreController } from "../ignore/RooIgnoreController"
import { RooProtectedController } from "../protect/RooProtectedController"
import { type AssistantMessageContent, presentAssistantMessage } from "../assistant-message"
import { NativeToolCallParser } from "../assistant-message/NativeToolCallParser"
import { manageContext, willManageContext, truncateConversation } from "../context-management"
import { ClineProvider } from "../webview/ClineProvider"
import { MultiSearchReplaceDiffStrategy } from "../diff/strategies/multi-search-replace"
import { MemoryManager } from "../../services/memory/MemoryManager"
import { parseTokenBudget, type TokenBudget } from "./TokenBudgetParser"
import {
	type ApiMessage,
	readApiMessages,
	saveApiMessages,
...
/**
 * 骞惰瀛愪换鍔＄殑鐘舵€併€? * Requirements: 13.4
 */
export interface ParallelChildState {
	taskId: string
	description: string
...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	progressSummarizer: TaskProgressSummarizer
	...
	/**
	 * 瑙勫垝妯″紡鏍囧織銆?	 * 褰?planMode 涓?true 鏃讹紝鍙厑璁稿彧璇诲伐鍏凤紙read_file/search_files/list_files 绛夛級锛?	 * 绂佹鍐欏叆鍜屾墽琛岀被宸ュ叿銆?	 * Requirements: 8.1, 8.2
	 */
	planMode: boolean = false

	/**
	 * 褰撳墠浠诲姟鍏宠仈鐨勬椿璺冭鍒掓枃浠跺悕锛堜笉鍚洰褰曡矾寰勶級銆?	 * 鍦?exit_plan_mode 淇濆瓨璁″垝鏃惰缃紝鍦?attempt_completion 鏃剁敤浜庢爣璁拌鍒掍负 completed銆?	 */
	activePlanFilename?: string

	/**
	 * 褰撳墠浠诲姟鐨?Token 棰勭畻銆?	 * 浠庣敤鎴锋秷鎭腑瑙ｆ瀽 "+500k"銆?+2M"銆?use 1M tokens" 绛夋牸寮忋€?	 * 瑙ｆ瀽鍚庡簲鐢ㄥ埌 API 璋冪敤鐨?modelMaxThinkingTokens 鍙傛暟銆?	 * Requirements: 11.1, 11.2, 11.3
	 */
	currentTokenBudget: TokenBudget | null = null

	/**
	 * 骞惰瀛愪换鍔＄姸鎬佽拷韪€?	 * 璁板綍褰撳墠浠诲姟鍒涘缓鐨勬墍鏈夊苟琛屽瓙浠诲姟鍙婂叾鐘舵€併€?	 * 鐢ㄤ簬闄愬埗鏈€澶у苟鍙戞暟锛堥粯璁?3锛夊拰灞曠ず瀛愪换鍔¤繘搴︺€?	 * Requirements: 13.4
	 */
	parallelChildren: Map<string, ParallelChildState> = new Map()

	/**
	 * 鏍囪瘑姝や换鍔℃槸鍚︿负骞惰瀛愪换鍔°€?	 * 骞惰瀛愪换鍔′笉璧?delegation 娴佺▼锛坅ttempt_completion 鏃剁洿鎺ュ畬鎴愶紝涓嶆仮澶嶇埗浠诲姟锛夈€?	 */
	isParallelChild: boolean = false

	/**
	 * 鏈€澶у苟琛屽瓙浠诲姟鏁帮紙鍙€氳繃璁剧疆璋冩暣锛岄粯璁?3锛夈€?	 * Requirements: 13.4
	 */
	static readonly DEFAULT_MAX_PARALLEL_CHILDREN = 10
	maxParallelChildren: number = Task.DEFAULT_MAX_PARALLEL_CHILDREN

	/**
	 * 骞惰浠诲姟瀹屾垚鏃剁殑 resolve 鍥炶皟銆?	 * spawn_parallel_task 宸ュ叿璋冪敤鏃惰缃紝鎵€鏈夊瓙浠诲姟瀹屾垚鍚庣敱 ClineProvider 璋冪敤銆?	 */
	parallelResolve?: (result: string) => void

	/**
	 * P2: 骞惰瀛愪换鍔＄殑瀵硅瘽鍘嗗彶蹇収锛堢敤浜庢煡鐪嬪瓙浠诲姟鎵ц璇︽儏锛夈€?	 */
	parallelChildrenHistory?: Array<{
		taskId: string
		description: string
		messages: ClineMessage[]
		costSummary?: { totalCostUSD: number }
	}>

	/**
	 * P2: 骞惰瀛愪换鍔¤嚜鍔ㄩ噸璇曡鏁般€?	 */
	...
	// 姣忎釜 Task 瀹炰緥鎸佹湁鐙珛鐨?NativeToolCallParser锛岄伩鍏嶅苟琛屽瓙浠诲姟鍏变韩 static 鐘舵€佸鑷寸珵鎬?
	public nativeToolParser = new NativeToolCallParser()
	...
	constructor({
		...
		this.progressSummarizer = new TaskProgressSummarizer()
		this.hooksRunner = new HooksRunner(this.workspacePath)
	...
	public async flushPendingToolResultsToHistory(): Promise<boolean> {
		...
		if (!this.assistantMessageSavedToHistory) {
			...
			// 瓒呮椂鎴?abort 鍚庨兘涓嶅簲缁х画鍐欏叆锛岄伩鍏嶅湪涓嶄竴鑷寸姸鎬佷笅姹℃煋鍘嗗彶璁板綍
			if (timedOut || this.abort) {
				return false
			}
		}
	...
	/**
	 * 鑾峰彇杈呭姪妯″瀷鐨?API handler锛堢敤浜?condense/鎽樿/璁板繂鎻愬彇绛夊悗鍙颁换鍔★級銆?	 * 濡傛灉鐢ㄦ埛閰嶇疆浜?lightModelId锛岃繑鍥炶交閲忔ā鍨?handler锛涘惁鍒欒繑鍥?null銆?	 * Requirements: 17.1, 17.2, 17.3, 17.4, 17.5
	 */
	public getLightApiHandler(): ApiHandler | null {
		return buildLightModelHandler(this.apiConfiguration)
	}
	...
	public async condenseContext(): Promise<void> {
		...
		const {
			...
			apiHandler: this.getLightApiHandler() ?? this.api,
			systemPrompt,
			taskId: this.taskId,
		...
		// Condense 鎴愬姛鍚庡悓姝ュ帇缂╄蹇嗭紝鍑忓皯鍚庣画绯荤粺鎻愮ず鐨?token 鍗犵敤
		await this.condenseMemoryAfterContextCondense()
	...
	/**
	 * 鎵嬪姩寮哄埗鎴柇涓婁笅鏂囷細鐩存帴璋冪敤 sliding window 涓㈠純鍓?50% 鍘嗗彶娑堟伅銆?	 * 涓嶄緷璧?LLM锛屼笉浼氬洜瓒呯獥澶辫触銆傜敤浜庣敤鎴蜂富鍔ㄦ仮澶嶅崱姝荤殑浠诲姟銆?	 */
	public async forceTruncateContext(): Promise<void> {
		await this.flushPendingToolResultsToHistory()

		const { contextTokens: prevContextTokens } = this.getTokenUsage()

		const truncationResult = truncateConversation(this.apiConversationHistory, 0.5, this.taskId)

		if (truncationResult.messages !== this.apiConversationHistory) {
			await this.overwriteApiConversationHistory(truncationResult.messages)
		}

		const contextTruncation: ContextTruncation = {
			truncationId: truncationResult.truncationId,
			messagesRemoved: truncationResult.messagesRemoved ?? 0,
			prevContextTokens,
			newContextTokens: 0, // Will be recalculated on next API request
		}
		await this.say(
	...
	private async getEnabledMcpToolsCount(): Promise<{ enabledToolCount: number; enabledServerCount: number }> {
		try {
			...
			if (!(mcpEnabled ?? true) || process.env.TOCODEX_DESKTOP_SHELL === "1") {
				return { enabledToolCount: 0, enabledServerCount: 0 }
			}
	...
	private async startTask(task?: string, images?: string[]): Promise<void> {
		try {
			...
			// 浠?provider state 鍔犺浇鍗曚换鍔℃垚鏈绠楋紙Requirements: 6.3锛?
			try {
				const provider = this.providerRef.deref()
				const state = provider ? await provider.getState() : undefined
				if (state?.taskCostBudget && state.taskCostBudget > 0) {
					this.costTracker.setBudget(state.taskCostBudget)
				}
			} catch (error) {
				console.warn(`[Task#${this.taskId}] 鍔犺浇璐圭敤棰勭畻澶辫触:`, error)
			}

			// 鍚姩杩涘害鎽樿鍣紙R3锛?
			this.progressSummarizer.start((summary: string | null) => {
				const provider = this.providerRef.deref()
				if (provider) {
			...
			// 瑙ｆ瀽鐢ㄦ埛娑堟伅涓殑 Token 棰勭畻锛圧11锛?
			if (task) {
				this.currentTokenBudget = parseTokenBudget(task)
			}
	...
	private async resumeTaskFromHistory() {
		try {
			...
			// 浠庡凡鏈夋秷鎭巻鍙叉仮澶嶈嚜鍔ㄥ鎵硅拷韪姸鎬侊紝
			// 閬垮厤鎭㈠鍚?lastResetMessageIndex 涓?0 瀵艰嚧绔嬪嵆瑙﹀彂闄愬埗銆?
			this.autoApprovalHandler.restoreFromMessages(this.clineMessages)
			...
			if (existingApiConversationHistory.length > 0) {
				...
				if (lastMessage.isSummary) {
					...
					// See: https://github.com/tocodex-ai/tocodex-community/issues/11487
			...
			if (responseText) {
				// 瑙ｆ瀽鐢ㄦ埛娑堟伅涓殑 Token 棰勭畻锛圧11锛?
				this.currentTokenBudget = parseTokenBudget(responseText)
	...
	public async abortTask(isAbandoned = false) {
		...
		// 閲嶈锛氬厛淇濆瓨娑堟伅锛屽啀 dispose銆?		// dispose 浼氳皟鐢?removeAllListeners 骞舵竻鐞嗚祫婧愶紙鍖呮嫭 providerRef锛夛紝
		// 涔嬪悗 saveClineMessages 鍐呴儴鐨?providerRef.deref() 鍙兘杩斿洖 undefined锛?		// 瀵艰嚧鍘嗗彶璁板綍鍜?UI 鐘舵€佹洿鏂伴潤榛樺け璐ャ€?
		try {
			await this.saveClineMessages()
		} catch (error) {
	...
	public dispose(): void {
		...
		// 浠诲姟缁撴潫鏃舵竻绌烘枃浠惰鍙栫紦瀛?
		try {
			FileReadCache.getInstance().clear()
		} catch (error) {
			console.error("Error clearing file read cache:", error)
		}

		// 鍋滄杩涘害鎽樿鍣紙R3.3锛?
		try {
			this.progressSummarizer.stop()
		} catch (error) {
			console.error("Error stopping progress summarizer:", error)
		}
	...
	// Parallel Children Management (Requirements: 13.4)

	/**
	 * 娉ㄥ唽涓€涓苟琛屽瓙浠诲姟銆?	 * 鍦ㄥ垱寤哄瓙浠诲姟鍓嶈皟鐢紝鐢ㄤ簬杩借釜鐘舵€佸拰闄愬埗骞跺彂鏁般€?	 */
	addParallelChild(taskId: string, description: string, files?: string[]): void {
		this.parallelChildren.set(taskId, {
			taskId,
			description,
			status: "running",
			startedAt: Date.now(),
	...
	/**
	 * 鏇存柊骞惰瀛愪换鍔＄殑鐘舵€併€?	 * 瀛愪换鍔″畬鎴愭垨澶辫触鏃惰皟鐢ㄣ€?	 */
	updateParallelChildStatus(taskId: string, status: "completed" | "failed", resultOrError?: string): void {
		const child = this.parallelChildren.get(taskId)
		if (!child) {
			return
		}
		child.status = status
		...
		if (status === "completed") {
			child.result = resultOrError
		} else {
			child.error = resultOrError
		}
	}

	/**
	 * 鑾峰彇褰撳墠姝ｅ湪杩愯鐨勫苟琛屽瓙浠诲姟鏁伴噺銆?	 */
	getRunningParallelChildrenCount(): number {
		let count = 0
		for (const child of this.parallelChildren.values()) {
	...
	/**
	 * 妫€鏌ユ槸鍚﹀彲浠ュ垱寤烘柊鐨勫苟琛屽瓙浠诲姟锛堟湭瓒呰繃鏈€澶у苟鍙戞暟锛夈€?	 */
	canSpawnParallelChild(): boolean {
		return this.getRunningParallelChildrenCount() < this.maxParallelChildren
	}

	/**
	 * 鑾峰彇鎵€鏈夊苟琛屽瓙浠诲姟鐨勭姸鎬佸揩鐓с€?	 */
	getParallelChildrenSnapshot(): ParallelChildState[] {
		return Array.from(this.parallelChildren.values())
	}
	...
	public async resumeAfterDelegation(): Promise<void> {
		...
		const { stableDetails: envDetails, volatileDetails } = await getEnvironmentDetails(this, true, true)
		...
		if (lastUserMsgIndex >= 0) {
			...
			if (Array.isArray(lastUserMsg.content)) {
				// Remove any existing environment_details and volatile_context blocks before adding fresh ones
				const contentWithoutEnvDetails = lastUserMsg.content.filter(
					(block: Anthropic.Messages.ContentBlockParam) => {
						if (block.type === "text" && typeof block.text === "string") {
							...
							const isVolatileContextBlock =
								trimmed.startsWith("<volatile_context>") && trimmed.endsWith("</volatile_context>")
							return !isEnvironmentDetailsBlock && !isVolatileContextBlock
						}
						return true
					},
				)
				// Add fresh environment details as separate blocks for prompt caching
				lastUserMsg.content = [
					...contentWithoutEnvDetails,
					{ type: "text" as const, text: envDetails },
					...(volatileDetails ? [{ type: "text" as const, text: volatileDetails }] : []),
				]
			}
		}
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			// 骞惰瀛愪换鍔′笉鏇存柊鍏ㄥ眬鏃堕棿鎴筹紝閬垮厤涓茶鍖栧叾浠栦换鍔＄殑璇锋眰
			await this.maybeWaitForProviderRateLimit(currentItem.retryAttempt ?? 0)
			if (!this.isParallelChild) {
				Task.lastGlobalApiRequestTime = performance.now()
			}

			// 妫€鏌?CostTracker 棰勭畻鐘舵€侊紙Requirements: 6.3, 6.4锛?			// 鍦ㄥ彂璧?API 璇锋眰鍓嶆鏌ワ紝閬垮厤瓒呭嚭鐢ㄦ埛璁惧畾鐨勫崟浠诲姟鎴愭湰棰勭畻
			const budgetStatus = this.costTracker.checkBudget()
			if (budgetStatus === "exceeded") {
				const summary = this.costTracker.formatSummary()
				const { response } = await this.ask(
					"auto_approval_max_req_reached",
					JSON.stringify({ count: this.costTracker.getTotalCost().toFixed(4), type: "cost" }),
				)
				if (response !== "yesButtonClicked") {
					// 鐢ㄦ埛鎷掔粷缁х画锛岀粓姝换鍔″惊鐜?					break
				}
				// 鐢ㄦ埛纭缁х画锛屼笉閲嶇疆棰勭畻锛堝厑璁哥户缁繍琛岋級
			} else if (budgetStatus === "warning") {
				console.warn(`[Task#${this.taskId}] 璐圭敤宸茶揪棰勭畻 80%: ${this.costTracker.formatSummary()}`)
			}
			...
			const { stableDetails: envDetails, volatileDetails } = await getEnvironmentDetails(
				this,
			...
			// Add environment details as separate text blocks:
			// - stableDetails (environment_details): 绋冲畾鍐呭锛屽彲琚?prompt caching 缂撳瓨
			// - volatileDetails (volatile_context): 鏄撳彉鍐呭锛堟椂闂存埑銆乧ost锛夛紝鏀惧湪鏈€鍚庝笉褰卞搷缂撳瓨鍓嶇紑
			...
			try {
				...
				this.nativeToolParser.clearAllStreamingToolCallsInstance()
				this.nativeToolParser.clearRawChunkStateInstance()
				...
				try {
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "usage":
								...
								// 杈撳嚭缂撳瓨鍛戒腑鏃ュ織鍒?Output Channel锛屾柟渚胯皟璇?prompt caching 鏁堟灉
								if (chunk.cacheWriteTokens || chunk.cacheReadTokens) {
									this.providerRef
										.deref()
										?.log(
											`[Prompt Cache] write=${chunk.cacheWriteTokens ?? 0} read=${chunk.cacheReadTokens ?? 0} input=${chunk.inputTokens} output=${chunk.outputTokens}`,
										)
								}
							...
							case "tool_call_partial": {
								...
								const events = this.nativeToolParser.processRawChunkInstance({
									index: chunk.index,
									id: chunk.id,
									name: chunk.name,
								...
								for (const event of events) {
									if (event.type === "tool_call_start") {
										...
										this.nativeToolParser.startStreamingToolCallInstance(
											event.id,
										...
										const partialToolUse = this.nativeToolParser.processStreamingChunkInstance(
											event.id,
											event.delta,
										)
										...
										const finalToolUse = this.nativeToolParser.finalizeStreamingToolCallInstance(
											event.id,
										)
					...
					const drainStreamInBackgroundToFindAllUsage = async (apiReqIndex: number) => {
						...
						const captureUsageData = async (
							...
							if (
								...
								// 璁板綍鍒?CostTracker 杩涜绮剧粏璐圭敤杩借釜 (Requirements: 6.1)
								const costUsage: TokenUsageForCost = {
									inputTokens: costResult.totalInputTokens,
									outputTokens: costResult.totalOutputTokens,
									cacheReadTokens: tokens.cacheRead,
									cacheWriteTokens: tokens.cacheWrite,
								}
								const costPricing: ModelPricing = {
									inputPricePerMToken: streamModelInfo.inputPrice ?? 0,
									outputPricePerMToken: streamModelInfo.outputPrice ?? 0,
									cacheReadPricePerMToken: streamModelInfo.cacheReadsPrice ?? 0,
									cacheWritePricePerMToken: streamModelInfo.cacheWritesPrice ?? 0,
								}
								this.costTracker.recordApiCall(modelId || "unknown", costUsage, costPricing)
					...
					if (!this.abandoned) {
						...
						const rawErrorDetails = (error as any)?.errorDetails
						...
						if (this.abort) {
							...
							console.error(
								`[Task#${this.taskId}.${this.instanceId}] Stream error details: name=${error?.constructor?.name} message=${error?.message} code=${(error as any)?.code} status=${(error as any)?.status} type=${(error as any)?.type}`,
							)
				...
				const finalizeEvents = this.nativeToolParser.finalizeRawChunksInstance()
				for (const event of finalizeEvents) {
					if (event.type === "tool_call_end") {
						// Finalize the streaming tool call
						const finalToolUse = this.nativeToolParser.finalizeStreamingToolCallInstance(event.id)
	...
	private async getSystemPrompt(): Promise<string> {
		...
		const isDesktopShell = process.env.TOCODEX_DESKTOP_SHELL === "1"
		let mcpHub: McpHub | undefined
		if ((mcpEnabled ?? true) && !isDesktopShell) {
			const provider = this.providerRef.deref()
		...
		return await (async () => {
			...
			return SYSTEM_PROMPT(
				...
				this.apiConversationHistory.length === 0, // isFirstTurn
			)
		})()
	}
	...
	/**
	 * Condense 鎴愬姛鍚庡悓姝ュ帇缂╄蹇嗗唴瀹广€?	 * 浣跨敤杈呭姪妯″瀷锛堝鏈夛級鎴栫畝鍗曟埅鏂潵鍑忓皯璁板繂鍗犵敤鐨?token 鏁般€?	 * 澶辫触鏃堕潤榛橀檷绾э紝涓嶅奖鍝嶄富娴佺▼銆?	 */
	private async condenseMemoryAfterContextCondense(): Promise<void> {
		try {
			const memoryManager = new MemoryManager({ cwd: this.cwd })
			const currentMemory = await memoryManager.loadMemory()
			if (!currentMemory) return

			const lightHandler = this.getLightApiHandler()
			const llmCondenser = lightHandler
				? async (memory: string) => {
						const prompt =
							"Condense the following project memory into the most important durable insights. Preserve architectural decisions, important paths, known issues, and key constraints. Remove redundant or outdated details. Output only the condensed memory."
						let result = ""
						const stream = lightHandler.createMessage(prompt, [{ role: "user", content: memory }])
			...
			await memoryManager.condenseMemory(llmCondenser)
		} catch (error) {
			// 闈欓粯闄嶇骇锛岃蹇嗗帇缂╁け璐ヤ笉搴旈樆濉?condense 娴佺▼
			console.warn(
				`[Task#${this.taskId}] 璁板繂鍘嬬缉澶辫触: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
	}
	...
	private async handleContextWindowExceededError(): Promise<void> {
		...
		try {
			...
			if (truncateResult.summary) {
				...
				// Condensing failed and we fell back to hard token-based truncation.
				// Surface a clear, user-facing error so the user knows why the conversation history shrank.
	...
	private async maybeWaitForProviderRateLimit(retryAttempt: number): Promise<void> {
		// 骞惰瀛愪换鍔¤烦杩囧叏灞€ rate limit锛岄伩鍏嶈涓茶鍖?		// 骞惰瀛愪换鍔″悇鑷嫭绔嬭姹?API锛屼笉搴斾簰鐩搁樆濉?
		if (this.isParallelChild) {
			return
		}
	...
	public async *attemptApiRequest(
		...
		this.providerRef
			.deref()
			?.log(
				`[Diag#attemptApiRequest] task=${this.taskId}.${this.instanceId} retry=${retryAttempt} stateProvider=${state?.apiConfiguration?.apiProvider}, taskProvider=${this.apiConfiguration?.apiProvider}, stateModel=${state?.apiConfiguration ? getModelId(state.apiConfiguration) : undefined}, taskModel=${this.apiConfiguration ? getModelId(this.apiConfiguration) : undefined}, hasRooKey=${!!state?.apiConfiguration?.rooApiKey}`,
			)
		...
		// 搴旂敤鐢ㄦ埛娑堟伅涓殑 Token 棰勭畻鍒?API 閰嶇疆锛圧11锛?
		if (this.currentTokenBudget && apiConfiguration) {
			const budgetConfig = {
				...apiConfiguration,
				modelMaxThinkingTokens: this.currentTokenBudget.tokens,
			}
			this.apiConfiguration = budgetConfig
			this.api = buildApiHandler(budgetConfig)
		}
		...
		if (contextTokens) {
			...
			try {
				...
				// Only surface condense error when there was no fallback truncation.
				// When truncation DID occur, the error is already embedded in the truncation notice.
				if (truncateResult.error && !truncateResult.truncationId) {
					await this.say("condense_context_error", truncateResult.error)
				}
				if (truncateResult.summary) {
		...
		this.providerRef
			.deref()
			?.log(
				`[Diag#attemptApiRequest] before createMessage task=${this.taskId}.${this.instanceId} provider=${this.apiConfiguration?.apiProvider}, model=${this.api ? this.api.getModel().id : undefined}, history=${cleanConversationHistory.length}, tools=${metadata.tools?.length ?? 0}`,
			)
		...
		this.providerRef
			.deref()
			?.log(`[Diag#attemptApiRequest] stream iterator created task=${this.taskId}.${this.instanceId}`)
		...
		try {
			...
			// Race between the first chunk, the abort signal, and a bounded first-byte timeout.
			// Without this, a provider/proxy that accepts the request but never sends SSE data leaves
			// the UI stuck forever on the placeholder api_req_started message.
			...
			const firstChunkTimeoutMs = Math.min(getApiRequestTimeout() ?? 60_000, 60_000)
			...
			const firstChunkTimeoutPromise = new Promise<never>((_, reject) => {
				firstChunkTimeout = setTimeout(() => {
					reject(new Error(`API request did not receive first chunk within ${firstChunkTimeoutMs}ms`))
				}, firstChunkTimeoutMs)
			})

			const firstChunk = await Promise.race([firstChunkPromise, abortPromise, firstChunkTimeoutPromise]).finally(
				() => {
		...
		// stream. Race it with the abort signal so Stop can break providers that
		// hang mid-stream without sending more chunks.
		while (true) {
			const nextPromise = iterator.next()
			const abortPromise = new Promise<never>((_, reject) => {
				if (abortSignal.aborted) {
					...
					abortSignal.addEventListener("abort", () => reject(new Error("Request cancelled by user")), {
						once: true,
					})
				}
			})
	...
	public recordToolUsage(toolName: ToolName, params?: Record<string, unknown>) {
		...
		// 璁板綍宸ュ叿璋冪敤鍒拌繘搴︽憳瑕佸櫒锛圧3.2锛?
		if (params) {
			this.progressSummarizer.recordToolCall(toolName, params)
		}
	}
