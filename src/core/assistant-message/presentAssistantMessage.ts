...
import { spawnParallelTaskTool } from "../tools/SpawnParallelTaskTool"
...
import {
	...
	getScheduledTaskRunsTool,
} from "../tools/ScheduledTaskTools"
 ...
 */
const RETRYABLE_TOOL_CONFIG: Record<string, { fallbackTool?: string }> = {
	apply_diff: { fallbackTool: "write_to_file" },
	apply_patch: { fallbackTool: "write_to_file" },
	execute_command: {},
	write_to_file: {},
	edit: {},
	search_and_replace: {},
...
/**
 * 在工具执行后检查是否失败，并注入重试上下文提示。
 * 如果工具失败且错误可重试，向用户消息内容追加提示让模型尝试不同方法。
 * Requirements: 5.1, 5.3, 5.4
 */
function injectRetryContextIfNeeded(
	cline: Task,
	toolName: string,
	failedBefore: boolean,
	mistakeCountBefore: number,
): void {
	// 工具执行前没有失败，执行后失败了 → 本次工具调用失败
	const failedNow = cline.didToolFailInCurrentTurn && !failedBefore
	const mistakeIncreased = cline.consecutiveMistakeCount > mistakeCountBefore

	if (!failedNow && !mistakeIncreased) {
		return
	}

	const config = RETRYABLE_TOOL_CONFIG[toolName]
	if (!config) {
		return
	}

	// 构建重试提示
	let retryHint = `[RetryStrategy] 工具 "${toolName}" 执行失败。`

	if (toolName === "apply_diff" && config.fallbackTool) {
		retryHint += ` 建议：如果 apply_diff 持续失败，请尝试使用 ${config.fallbackTool} 替代。`
	} else if (toolName === "apply_patch" && config.fallbackTool) {
		retryHint += ` 建议：如果 apply_patch 持续失败，请尝试使用 ${config.fallbackTool} 替代。`
	} else if (toolName === "execute_command") {
		retryHint += ` 建议：请检查命令输出中的错误信息，尝试修正后重新执行。`
	} else {
		retryHint += ` 建议：请尝试不同的方法来完成此操作。`
	}

	// 将重试提示追加到用户消息内容，让模型在下一轮看到
		...
		case "tool_use": {
			...
			if (!toolCallId) {
				...
				try {
					...
					console.warn(`[presentAssistantMessage] recordToolError failed for invalid tool call:`, error)
				}
			...
			const toolDescription = (): string => {
				switch (block.name) {
					...
					case "lsp_code_intelligence":
						return `[${block.name} ${block.params.operation ?? ""}${block.params.filePath ? ` on '${block.params.filePath}'` : ""}]`
					...
					case "tool_search":
						return `[${block.name} for '${block.params.query}']`
					case "notebook_edit":
						return `[${block.name} ${block.params.operation ?? "edit"} cell ${block.params.cellIndex ?? "?"} in '${block.params.path}']`
					default:
						return `[${block.name}]`
				}
			}
			...
			if (!block.partial) {
				...
				if (isKnownTool && !block.nativeArgs && !customTool) {
					...
					try {
						...
						console.warn(`[presentAssistantMessage] recordToolError failed for missing nativeArgs:`, error)
					}
			...
			if (!block.partial) {
				...
				cline.recordToolUsage(recordName, block.params as Record<string, unknown>)
			...
			if (!block.partial) {
				...
				if (cline.planMode) {
					...
					if (!PLAN_MODE_ALLOWED_TOOLS.includes(block.name)) {
						const errorMessage = `Tool "${block.name}" is not allowed in plan mode. Only read-only tools are permitted: ${PLAN_MODE_ALLOWED_TOOLS.join(", ")}. Use exit_plan_mode to exit plan mode first.`
						cline.consecutiveMistakeCount++
						const errorContent = formatResponse.toolError(errorMessage)
						cline.pushToolResultToUserContent({
							type: "tool_result",
							tool_use_id: sanitizeToolUseId(toolCallId),
							content: typeof errorContent === "string" ? errorContent : "(plan mode violation)",
							is_error: true,
						})
						break
					}
			...
			// ── PreToolUse Hooks (Requirements: 15.1) ──
			// 在工具执行前运行用户配置的 hook 命令
			if (!block.partial && cline.hooksRunner) {
				try {
					const preHookResults = await cline.hooksRunner.runPreToolUse(block.name, block.params)
					for (const hr of preHookResults) {
						if (hr.injectedContext) {
							cline.userMessageContent.push({ type: "text" as const, text: hr.injectedContext })
						}
					}
				} catch (error) {
					console.warn(`[presentAssistantMessage] preToolUse hook failed for ${block.name}:`, error)
				}
			}

			switch (block.name) {
				case "write_to_file": {
					const failedBefore = cline.didToolFailInCurrentTurn
					const mistakesBefore = cline.consecutiveMistakeCount
					...
					injectRetryContextIfNeeded(cline, "write_to_file", failedBefore, mistakesBefore)
					break
				}
				case "update_todo_list":
				...
				case "apply_diff": {
					...
					injectRetryContextIfNeeded(cline, "apply_diff", failedBefore, mistakesBefore)
					break
				}
				case "edit":
				case "search_and_replace": {
					...
					injectRetryContextIfNeeded(cline, block.name, failedBefore, mistakesBefore)
					break
				}
				case "search_replace": {
					const failedBefore = cline.didToolFailInCurrentTurn
					...
					injectRetryContextIfNeeded(cline, "search_replace", failedBefore, mistakesBefore)
					break
				}
				case "edit_file": {
					const failedBefore = cline.didToolFailInCurrentTurn
					...
					injectRetryContextIfNeeded(cline, "edit_file", failedBefore, mistakesBefore)
					break
				}
				case "apply_patch": {
					const failedBefore = cline.didToolFailInCurrentTurn
					...
					injectRetryContextIfNeeded(cline, "apply_patch", failedBefore, mistakesBefore)
					break
				}
				case "read_file":
				...
				case "codebase_search":
					...
					break
				case "lsp_code_intelligence":
					await lspTool.handle(cline, block as ToolUse<"lsp_code_intelligence">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "spawn_parallel_task":
					await checkpointSaveAndMark(cline)
					await spawnParallelTaskTool.handle(cline, block as ToolUse<"spawn_parallel_task">, {
						askApproval,
						handleError,
						pushToolResult,
						toolCallId: block.id,
					...
					break
				case "tool_search":
					await toolSearchTool.handle(cline, block as ToolUse<"tool_search">, {
						askApproval,
						handleError,
						pushToolResult,
					})
					break
				case "notebook_edit":
					await notebookEditTool.handle(cline, block as ToolUse<"notebook_edit">, {
						askApproval,
						handleError,
						pushToolResult,
					})
					break
				case "schedule_task":
					await scheduleTaskTool.handle(cline, block as ToolUse<"schedule_task">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "list_scheduled_tasks":
					await listScheduledTasksTool.handle(cline, block as ToolUse<"list_scheduled_tasks">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "update_scheduled_task":
					await updateScheduledTaskTool.handle(cline, block as ToolUse<"update_scheduled_task">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "cancel_scheduled_task":
					await cancelScheduledTaskTool.handle(cline, block as ToolUse<"cancel_scheduled_task">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "run_scheduled_task_now":
					await runScheduledTaskNowTool.handle(cline, block as ToolUse<"run_scheduled_task_now">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "get_scheduled_task_runs":
					await getScheduledTaskRunsTool.handle(cline, block as ToolUse<"get_scheduled_task_runs">, {
						askApproval,
						handleError,
						pushToolResult,
					})
				...
				case "execute_command": {
					...
					injectRetryContextIfNeeded(cline, "execute_command", failedBefore, mistakesBefore)
					break
				}
				case "read_command_output":
			...
			// ── PostToolUse Hooks (Requirements: 15.2) ──
			// 在工具执行后运行用户配置的 hook 命令
			if (!block.partial && cline.hooksRunner) {
				try {
					const postHookResults = await cline.hooksRunner.runPostToolUse(block.name, block.params)
					for (const hr of postHookResults) {
						if (hr.injectedContext) {
							cline.userMessageContent.push({ type: "text" as const, text: hr.injectedContext })
						}
					}
				} catch (error) {
					console.warn(`[presentAssistantMessage] postToolUse hook failed for ${block.name}:`, error)
				}
			}
