...
export class NativeToolCallParser {
	...
	// ── 实例级状态：每个 Task 持有独立的解析器实例，避免并行子任务间的竞态 ──
	private instanceStreamingToolCalls = new Map<
		string,
		{
			id: string
			name: string
	...
	private instanceRawChunkTracker = new Map<
		number,
		{
			id: string
			name: string
	...
	// ── 实例方法：每个 Task 持有独立的解析器实例，并行子任务不再共享全局 static 状态 ──

	/**
	 * 实例级：处理原始 tool call chunk，使用实例级状态避免并行任务竞态。
	 */
	public processRawChunkInstance(chunk: {
		index: number
		id?: string
		name?: string
		arguments?: string
		...
		let tracked = this.instanceRawChunkTracker.get(index)
		...
		if (id && !tracked) {
			...
			this.instanceRawChunkTracker.set(index, tracked)
		}
	...
	/**
	 * 实例级：finalize 所有未结束的 raw chunks。
	 */
	public finalizeRawChunksInstance(): ToolCallStreamEvent[] {
		const events: ToolCallStreamEvent[] = []

		if (this.instanceRawChunkTracker.size > 0) {
			for (const [, tracked] of this.instanceRawChunkTracker.entries()) {
				if (tracked.hasStarted) {
					events.push({
						type: "tool_call_end",
						id: tracked.id,
			...
			this.instanceRawChunkTracker.clear()
		}
	...
	/**
	 * 实例级：清除 raw chunk 追踪状态。
	 */
	public clearRawChunkStateInstance(): void {
		this.instanceRawChunkTracker.clear()
	}

	/**
	 * 实例级：开始流式 tool call 追踪。
	 */
	public startStreamingToolCallInstance(id: string, name: string): void {
		this.instanceStreamingToolCalls.set(id, {
			id,
			name,
			argumentsAccumulator: "",
		})
	...
	/**
	 * 实例级：清除所有流式 tool call 状态。
	 */
	public clearAllStreamingToolCallsInstance(): void {
		this.instanceStreamingToolCalls.clear()
	}

	/**
	 * 实例级：处理流式 JSON 参数 chunk。
	 */
	public processStreamingChunkInstance(id: string, chunk: string): ToolUse | null {
		const toolCall = this.instanceStreamingToolCalls.get(id)
		if (!toolCall) {
			return null
		}
	...
	/**
	 * 实例级：finalize 流式 tool call，返回最终的 ToolUse。
	 */
	public finalizeStreamingToolCallInstance(id: string): ToolUse | McpToolUse | null {
		const toolCall = this.instanceStreamingToolCalls.get(id)
		if (!toolCall) {
			return null
		}
		...
		this.instanceStreamingToolCalls.delete(id)
		...
		switch (name) {
			...
			case "lsp_code_intelligence":
				if (partialArgs.operation !== undefined) {
					nativeArgs = {
						...
						filePath: partialArgs.filePath ?? null,
						line: this.coerceOptionalNumber(partialArgs.line) ?? null,
						character: this.coerceOptionalNumber(partialArgs.character) ?? null,
						symbol: partialArgs.symbol ?? null,
					}
				}
				break
			...
			case "web_fetch":
				if (partialArgs.url !== undefined) {
					nativeArgs = {
						url: partialArgs.url,
						prompt: partialArgs.prompt ?? null,
						maxLength: this.coerceOptionalNumber(partialArgs.maxLength) ?? null,
					}
				}
				break
			...
			case "notebook_edit":
				if (partialArgs.path !== undefined) {
					nativeArgs = {
						path: partialArgs.path,
						operation: partialArgs.operation ?? "replace",
						cellIndex: this.coerceOptionalNumber(partialArgs.cellIndex) ?? 0,
						cellType: partialArgs.cellType ?? null,
						content: partialArgs.content ?? null,
					}
				}
				break
			...
			case "schedule_task":
				if (partialArgs.name !== undefined) {
					nativeArgs = {
						...
						description: partialArgs.description ?? null,
						trigger: partialArgs.trigger ?? null,
						mode: partialArgs.mode ?? null,
						prompt: partialArgs.prompt ?? null,
						apiConfigName: partialArgs.apiConfigName ?? null,
						scope: partialArgs.scope ?? null,
						maxRuntimeMs: this.coerceOptionalNumber(partialArgs.maxRuntimeMs) ?? null,
						concurrency: partialArgs.concurrency ?? null,
						notifyOn: partialArgs.notifyOn ?? null,
					}
				}
				break

			case "list_scheduled_tasks":
				// No required params, always emit
				nativeArgs = {
					scope: partialArgs.scope ?? null,
					enabled_only: partialArgs.enabled_only ?? null,
				}
				break
			...
			case "update_scheduled_task":
				if (partialArgs.id !== undefined) {
					nativeArgs = {
						id: partialArgs.id,
						name: partialArgs.name ?? null,
						description: partialArgs.description ?? null,
						trigger: partialArgs.trigger ?? null,
						mode: partialArgs.mode ?? null,
						prompt: partialArgs.prompt ?? null,
						apiConfigName: partialArgs.apiConfigName ?? null,
						enabled: partialArgs.enabled ?? null,
						maxRuntimeMs: this.coerceOptionalNumber(partialArgs.maxRuntimeMs) ?? null,
						concurrency: partialArgs.concurrency ?? null,
						notifyOn: partialArgs.notifyOn ?? null,
					}
			...
			case "get_scheduled_task_runs":
				if (partialArgs.id !== undefined) {
					nativeArgs = {
						id: partialArgs.id,
						limit: this.coerceOptionalNumber(partialArgs.limit) ?? null,
					}
				}
				break
				...
				case "lsp_code_intelligence":
					if (args.operation !== undefined) {
						nativeArgs = {
							...
							line: this.coerceOptionalNumber(args.line) ?? null,
							character: this.coerceOptionalNumber(args.character) ?? null,
				...
				case "web_fetch":
					if (args.url !== undefined) {
						nativeArgs = {
							...
							maxLength: this.coerceOptionalNumber(args.maxLength) ?? null,
						} as NativeArgsFor<TName>
					}
					break
				...
				case "notebook_edit":
					if (args.path !== undefined) {
						nativeArgs = {
							...
							cellIndex: this.coerceOptionalNumber(args.cellIndex) ?? 0,
							cellType: args.cellType ?? null,
