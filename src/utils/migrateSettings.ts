...
export async function migrateSettings(
	...
	// Migrate soundEnabled default: enable sound notifications by default for existing users
	await migrateSoundEnabledDefault(context, outputChannel)
	// Ensure default-on experiments are enabled for new and upgraded users
	await migrateImageGenerationExperimentDefault(context, outputChannel)
	// Ensure MCP server toggle is enabled for new and upgraded users
	await migrateMcpEnabledDefault(context, outputChannel)
...
/**
 * Migration: Enable sound notifications by default for existing users.
 * Previously the default was false; this one-time migration sets it to true
 * for users who have never explicitly changed the setting.
 */
async function migrateSoundEnabledDefault(
	context: vscode.ExtensionContext,
	outputChannel: vscode.OutputChannel,
): Promise<void> {
	try {
		const migrationKey = "soundEnabledDefaultMigrated"
		if (context.globalState.get(migrationKey)) {
			return
		}
		const current = context.globalState.get<boolean>("soundEnabled")
		// Only set to true if the user never explicitly saved a value (undefined)
		// or if it is still the old implicit default (false and never been touched)
		if (current === undefined || current === false) {
			await context.globalState.update("soundEnabled", true)
			outputChannel.appendLine("[SoundEnabled Migration] Set soundEnabled to true (new default)")
		}
		await context.globalState.update(migrationKey, true)
	} catch (error) {
		outputChannel.appendLine(`[SoundEnabled Migration] Error: ${error}`)
	}
}

/**
 * Migration: Enable default-on experiments for new and upgraded users.
 * Some installs may have no experiments object yet, and older versions may have persisted old defaults.
 * Force both imageGeneration and preventFocusDisruption to true so the settings are checked immediately after install/upgrade.
 */
async function migrateImageGenerationExperimentDefault(
	context: vscode.ExtensionContext,
	outputChannel: vscode.OutputChannel,
): Promise<void> {
	try {
		const experiments = context.globalState.get<Record<string, boolean>>("experiments")
		const currentExperiments =
			experiments && typeof experiments === "object" && !Array.isArray(experiments) ? experiments : {}
		...
		if (
			currentExperiments.imageGeneration === true &&
			currentExperiments.preventFocusDisruption === true &&
			experiments === currentExperiments
		) {
			return
		}

		await context.globalState.update("experiments", updatedExperiments)
		outputChannel.appendLine("[Experiments Migration] Enabled default-on experiments for install/upgrade defaults")
	} catch (error) {
		outputChannel.appendLine(`[Experiments Migration] Error: ${error}`)
	}
}

/**
 * Migration: Force mcpEnabled to true for new and upgraded users.
 * Ensures the MCP server toggle is always checked after install/upgrade.
 */
async function migrateMcpEnabledDefault(
	context: vscode.ExtensionContext,
	outputChannel: vscode.OutputChannel,
): Promise<void> {
	try {
		const mcpEnabled = context.globalState.get<boolean>("mcpEnabled")

		if (mcpEnabled === true) {
			return
		}

		await context.globalState.update("mcpEnabled", true)
		outputChannel.appendLine("[MCP Migration] Set mcpEnabled to true (forced default)")
	} catch (error) {
		outputChannel.appendLine(`[MCP Migration] Error: ${error}`)
	}
}
