...
export function consolidateTokenUsage(messages: ClineMessage[]): TokenUsage {
	...
	messages.forEach((message) => {
		if (message.type === "say" && message.say === "api_req_started" && message.text) {
			...
			// 累加上下文压缩节省的 tokens（压缩前 - 压缩后）
			const prev = message.contextCondense?.prevContextTokens ?? 0
			const next = message.contextCondense?.newContextTokens ?? 0
			if (prev > next) {
				result.condenseSavedTokens = (result.condenseSavedTokens ?? 0) + (prev - next)
			}
		}
	})
