...
export const globalSettingsSchema = z.object({
	...
	imageGenerationProvider: z.enum(["openrouter", "roo", "custom"]).optional(),
	openRouterImageApiKey: z.string().optional(),
	openRouterImageGenerationSelectedModel: z.string().optional(),
	// 自定义生图 provider 配置（独立于对话 provider）
	customImageBaseUrl: z.string().optional(),
	customImageApiKey: z.string().optional(),
	// 从 .tocodex/image-models.json 加载的自定义生图模型列表
	imageGenerationModels: z
		.array(
			z.object({
				value: z.string(),
				label: z.string(),
				provider: z.enum(["openrouter", "roo", "custom"]),
				apiMethod: z.enum(["chat_completions", "images_api"]).optional(),
			}),
		)
		.optional(),
	// 图片生成尺寸，格式 "WIDTHxHEIGHT" 如 "1024x1024"
	imageGenerationSize: z.string().optional(),
	...
	alwaysAllowAllCommands: z.boolean().optional(),
	...
	autoGenerateTaskMemo: z.boolean().optional(),
	allowedMaxRequests: z.number().nullish(),
	allowedMaxCost: z.number().nullish(),
	/**
	 * 单任务成本预算上限（美元），达到 80% 时警告，100% 时停止任务
	 */
	taskCostBudget: z.number().nullish(),
	...
	maxParallelChildren: z.number().int().min(1).max(10).optional(),
	...
	loadRecentPlans: z.boolean().optional(),
	...
	showCodebaseIndexInChat: z.boolean().optional(),
...
export const GLOBAL_SECRET_KEYS = [
	...
	"customImageApiKey", // For custom image generation provider
] as const
