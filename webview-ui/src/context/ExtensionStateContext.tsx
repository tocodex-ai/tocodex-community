...
export interface ExtensionStateContextType extends ExtensionState {
	...
	setAlwaysAllowAllCommands: (value: boolean) => void
	setAlwaysAllowMcp: (value: boolean) => void
	setAlwaysAllowModeSwitch: (value: boolean) => void
	setAlwaysAllowSubtasks: (value: boolean) => void
	setShowRooIgnoredFiles: (value: boolean) => void
	setEnableSubfolderRules: (value: boolean) => void
	setLoadRecentPlans: (value: boolean) => void
...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	const [state, setState] = useState<ExtensionState>({
		...
		loadRecentPlans: true, // Default to enabled - load recent plans from .tocodex/plans/ into context
		...
		(event: MessageEvent) => {
			...
			switch (message.type) {
				case "state": {
					...
					// For roo provider, also require cloudIsAuthenticated
					// For openai-codex provider, also require openAiCodexIsAuthenticated
					// This prevents cached apiProvider from bypassing the welcome page when not actually authenticated
					const apiProvider = newState.apiConfiguration?.apiProvider
					const hasValidConfig =
						checkExistKey(newState.apiConfiguration) &&
						(apiProvider !== "roo" || !!newState.cloudIsAuthenticated) &&
						(apiProvider !== "openai-codex" || !!newState.openAiCodexIsAuthenticated)
					setShowWelcome(!hasValidConfig)
	...
	const contextValue: ExtensionStateContextType = useMemo<ExtensionStateContextType>(
		() => ({
			...state,
			...
			setAlwaysAllowAllCommands: (value) =>
				setState((prevState) => ({ ...prevState, alwaysAllowAllCommands: value })),
			...
			setLoadRecentPlans: (value) => setState((prevState) => ({ ...prevState, loadRecentPlans: value })),
