...
export const ModeSelector = ({
	...
	// 支持 i18n 翻译的模式 slug 列表
	const defaultModeSlugs = React.useMemo(
		() =>
			new Set([
				"architect",
				"code",
	...
	const getLocalizedName = React.useCallback(
		(mode: ModeConfig) => {
			if (defaultModeSlugs.has(mode.slug)) {
				const key = `chat:modeSelector.modes.${mode.slug}.name`
				const translated = t(key)
				// 如果翻译 key 存在则使用翻译，否则回退到原始名称
	...
	const getLocalizedDescription = React.useCallback(
		(mode: ModeConfig & { description?: string }) => {
			if (defaultModeSlugs.has(mode.slug)) {
				const key = `chat:modeSelector.modes.${mode.slug}.description`
				const translated = t(key)
				if (translated !== key) return translated
			}
		...
		<Popover open={open} onOpenChange={onOpenChange} data-testid="mode-selector-root">
			<StandardTooltip content={title}>
				<PopoverTrigger
					...
					<span className="truncate">{selectedMode ? getLocalizedName(selectedMode) : ""}</span>
				</PopoverTrigger>
			</StandardTooltip>
			<PopoverContent
				align="start"
									...
									return (
										<div
											...
											<div className="flex-1 min-w-0">
												<div className="font-bold truncate">{getLocalizedName(mode)}</div>
												{mode.description && (
													<div className="text-xs text-vscode-descriptionForeground truncate">
														{getLocalizedDescription(mode)}
													</div>
												)}
											</div>
