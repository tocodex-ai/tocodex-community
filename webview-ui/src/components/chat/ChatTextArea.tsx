...
export const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		// 截图模式：点截图按钮后 60 秒内，输入框获得焦点时自动读取剪贴板图片
		const screenshotModeRef = useRef(false)
		const screenshotTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
		const [searchRequestId, setSearchRequestId] = useState<string>("")

		// window 级别 paste 监听：IDE webview 可能拦截 React onPaste，用捕获阶段绕过
		useEffect(() => {
			const onWindowPaste = (e: ClipboardEvent) => {
				...
				if (imageItems.length > 0 && !pastedText) {
					e.preventDefault()
					e.stopPropagation()
					...
					Promise.all(imagePromises).then((results) => {
						const dataUrls = results.filter((d): d is string => d !== null)
		...
		const updateHighlights = useCallback(() => {
			...
			// Token 预算关键词高亮（R11）
			// 匹配 "use Nk/M/B tokens"、"+Nk/M/B"、"Nk/M/B tokens" 等格式
			const budgetPatterns = [
				/\buse\s+\d+(?:\.\d+)?\s*[kKmMbB]?\s*tokens?\b/gi,
				/\+\d+(?:\.\d+)?\s*[kKmMbB]\b/g,
				/\b\d+(?:\.\d+)?\s*[kKmMbB]\s*tokens?\b/gi,
			]
			for (const pattern of budgetPatterns) {
				processedText = processedText.replace(pattern, '<mark class="token-budget-highlight">$&</mark>')
			}
			...
			<div
						...
						<div
							...
							<div
								...
								className={cn(
									...
									isFocused
										? "border border-vscode-focusBorder outline outline-vscode-focusBorder chat-input-focused"
										: isDraggingOver
							...
							<DynamicTextArea
								...
								onFocus={() => {
									setIsFocused(true)
									// 截图模式下自动读取剪贴板图片（读取一次后立即关闭，避免重复粘贴）
									if (screenshotModeRef.current && !shouldDisableImages) {
														...
														reader.onloadend = () => {
															...
															if (dataUrl) {
																setSelectedImages((prev) =>
																	[...prev, dataUrl].slice(0, MAX_IMAGES_PER_MESSAGE),
																)
															}
														}
														reader.readAsDataURL(blob)
							...
							<div className="absolute bottom-2 right-1 z-30 flex flex-col items-center gap-0">
								<StandardTooltip content={t("chat:pasteScreenshot")}>
									<button
										aria-label={t("chat:pasteScreenshot")}
										onClick={() => {
											// 调用截图工具
											vscode.postMessage({ type: "pasteImageFromClipboard" } as any)
											// 开启截图模式，60 秒内输入框获焦时自动读取剪贴板图片
				...
				<div className="flex items-center gap-2">
					...
					<div
						...
						{!isEditMode && showCodebaseIndexInChat ? <IndexingStatusBadge /> : null}
						{!isEditMode && (
							<button
								onClick={() => window.postMessage({ type: "action", action: "cloudButtonClicked" })}
								className="inline-flex items-center justify-center w-4.5 h-4.5 ml-1 rounded-full bg-transparent border border-vscode-dropdown-border opacity-50 hover:opacity-100 cursor-pointer"
								title={
									cloudUserInfo ? cloudUserInfo.name || t("cloud:personalAccount") : t("cloud:signIn")
								}>
								<span
									className={`codicon codicon-account text-xs ${cloudUserInfo ? "text-vscode-foreground" : "text-vscode-descriptionForeground"}`}
								/>
							</button>
						)}
					</div>
