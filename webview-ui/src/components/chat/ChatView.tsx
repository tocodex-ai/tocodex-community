...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	// 检测当前是否处于规划模式（从聊天消息中推断）
	const isPlanMode = useMemo(() => {
		// 从后往前扫描消息，找到最近的 enterPlanMode 或 exitPlanMode 工具消息
		for (let i = messages.length - 1; i >= 0; i--) {
			const msg = messages[i]
			if (msg.type === "say" && msg.say === "tool") {
				try {
					const toolData = JSON.parse(msg.text || "{}")
					if (toolData.tool === "exitPlanMode") {
						return false
					}
					if (toolData.tool === "enterPlanMode") {
						return true
					}
				} catch {
					// ignore parse errors
					...
					switch (lastMessage.ask) {
						...
						case "tool":
							...
							try {
								...
								// text 不是有效 JSON（如 URL 字符串），使用空对象兜底
						...
						case "completion_result":
							...
							setEnableButtons(!isPartial)
							setPrimaryButtonText(t("chat:continue.title"))
							setSecondaryButtonText(t("chat:startNewTask.title"))
							break
						case "resume_task":
							setSendingDisabled(false)
	...
	const handleSendMessage = useCallback(
		(text: string, images: string[]) => {
			...
			if (text || images.length > 0) {
				// 拦截 /remember 和 /forget 命令，直接发送到后端处理
				// Requirements: 2.4, 2.5 + 全局记忆扩展
				if (text.startsWith("/remember-global ")) {
					const memoryText = text.slice("/remember-global ".length).trim()
					if (memoryText) {
						vscode.postMessage({ type: "rememberGlobalMemory", text: memoryText })
					}
					setInputValue("")
					return
				}
				if (text === "/forget-global") {
					vscode.postMessage({ type: "forgetGlobalMemory" })
					setInputValue("")
					return
				}
				if (text.startsWith("/remember ")) {
					...
					if (memoryText) {
						vscode.postMessage({ type: "rememberMemory", text: memoryText })
					}
					setInputValue("")
					return
				}
				if (text === "/forget") {
					vscode.postMessage({ type: "forgetMemory" })
					setInputValue("")
					return
				}
		...
		(e: MessageEvent) => {
			...
			switch (message.type) {
				...
				case "progressSummary":
					setProgressSummary(message.text || null)
					break
			}
	...
	const handleForceTruncateContext = (taskId: string) => {
		if (isCondensing || sendingDisabled) {
			return
		}
		setIsCondensing(true)
		setSendingDisabled(true)
		vscode.postMessage({ type: "forceTruncateContext", text: taskId })
	}
			...
			{task ? (
				<>
					<TaskHeader
						...
						condenseSavedTokens={apiMetrics.condenseSavedTokens}
						...
						taskCostSummary={taskCostSummary}
						buttonsDisabled={sendingDisabled}
						handleCondenseContext={handleCondenseContext}
						handleForceTruncateContext={handleForceTruncateContext}
						todos={latestTodos}
						messages={messages}
					...
					<PlanModeBar isVisible={isPlanMode} buttonsDisabled={sendingDisabled || isStreaming} />

					{checkpointWarning && (
						<div className="px-3">
							<CheckpointWarning
								warning={checkpointWarning}
								onDismiss={() => setCheckpointWarning(undefined)}
							/>
						</div>
					)}
				</>
			) : (
				<div className="flex flex-col h-full min-h-0 overflow-y-auto welcome-glow-bg">
					{/* 版本号固定在右上角 */}
					<div className="flex justify-end p-2 pr-3 shrink-0">
						<VersionIndicator />
					</div>
					<div className="flex flex-col flex-1 min-h-0 px-6 pb-6">
						{taskHistory.length > 0 ? (
							/* 有任务时：内容从顶部开始布局，允许自然滚动 */
							<div className="flex flex-col min-[400px]:px-6 pt-2">
							...
							/* 无任务时：logo 居左上角，其余内容垂直居中 */
							<div className="flex flex-col h-full min-[400px]:px-6">
								<RooHero />
			...
			{task && (
				<>
					<div className="grow flex relative" ref={scrollContainerRef}>
						...
						/>
						{/* 滚动到底部按钮：使用绝对定位，不影响布局流，避免抖动 */}
						{showScrollToBottom && !primaryButtonText && !secondaryButtonText && (
					...
					{(primaryButtonText || secondaryButtonText) && (
						<div
							className={`flex h-9 items-center mb-1 px-[15px] shrink-0 ${
											...
											: primaryButtonText === t("chat:save.title")
												...
												: primaryButtonText === t("chat:approve.title")
													...
													: primaryButtonText === t("chat:runCommand.title")
														...
														: primaryButtonText === t("chat:continue.title")
															? t("chat:continue.tooltip")
