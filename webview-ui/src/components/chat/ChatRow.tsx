...
const ChatRow = memo(
	(props: ChatRowProps) => {
		...
		const [chatrow, { height }] = useSize(
			<div className="px-[15px] py-[10px] pr-[6px] animate-chat-message-in" style={{ contain: "layout" }}>
...
export const ChatRowContent = ({
			...
			case "command":
				return [
					...
					isCommandExecuting ? (
						...
						<span style={{ color: normalColor, fontWeight: "bold" }}>
							{t("chat:commandExecution.wantsToExecute")}
						</span>
					),
				]
			case "use_mcp_server":
	...
	if (tool) {
		...
		switch (tool.tool as string) {
				...
				return (
					<>
						<div style={headerStyle}>
							...
							/>
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask" ? t("chat:planMode.wantsToEnter") : t("chat:planMode.entered")}
							</span>
						</div>
						{tool.reason && (
					...
					<>
						<div style={headerStyle}>
							<ClipboardList
								...
								aria-label="Exit plan mode icon"
							/>
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask" ? t("chat:planMode.planReady") : t("chat:planMode.exited")}
							</span>
						</div>
						{tool.result && (
							<div className="text-muted-foreground pl-6">
								<MarkdownBlock markdown={tool.result} />
							</div>
						)}
					</>
				)
			...
			case "spawnParallelTask": {
				const spawnInfo = safeJsonParse<{
					tasks?: Array<{ description: string; mode?: string; files?: string[] }> | string
					summary?: string
					completedChildren?: Array<{
						taskId: string
						description: string
				...
				// tasks 可能是 JSON 字符串或数组
				let taskList: Array<{ description: string; mode?: string; files?: string[] }> = []
				if (spawnInfo?.tasks) {
					if (Array.isArray(spawnInfo.tasks)) {
						taskList = spawnInfo.tasks
					} else if (typeof spawnInfo.tasks === "string") {
						try {
							const parsed = JSON.parse(spawnInfo.tasks)
				...
				// 优先使用消息中保存的快照（历史数据），否则仅在当前消息是最新消息时使用实时状态
				// 避免历史消息中的旧 spawnParallelTask 行显示当前批次的实时数据
				const displayChildren =
					spawnInfo?.completedChildren ??
					(isLast ? parallelChildren : undefined) ??
					// 兜底：从 taskList 构造静态显示（无状态信息，但至少能看到子任务列表）
					(taskList.length > 0
						? taskList.map((t, i) => ({
								taskId: `task-${i}`,
				...
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask" ? "创建并行子任务" : "已派发并行子任务"}
							</span>
						</div>
						{displayChildren && displayChildren.length > 0 && (
							<div className="pl-6 mt-1">
								<ParallelTasksPanel children={displayChildren} />
							</div>
						)}
					</>
				)
	...
	switch (message.type) {
					...
					if (message.text !== undefined) {
						...
						if (!isNaN(potentialCode) && potentialCode >= 400) {
							...
							if (i18n.exists(stringForError)) {
								...
								docsURL =
									"mailto:support@tocodex.com?subject=Unknown API Error&body=[Please include full error details]"
							}
						}
							...
							return (
								<div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0" }}>
									...
									<span style={{ fontWeight: "bold", color: "var(--vscode-foreground)" }}>
										{sayTool.content || t("chat:fileOperations.generatingImage", { defaultValue: "Generating image, please wait..." })}
									</span>
								</div>
							)
						}
		...
		case "ask":
			switch (message.ask) {
				...
				case "tool": {
					// WebFetchTool 传的是纯 URL 字符串，JSON.parse 失败后 tool 为 null
					// 在这里渲染"需要访问网络"的描述
					const urlText = message.text || ""
						...
						return (
							<>
								<div style={headerStyle}>
									...
									/>
									<span style={{ fontWeight: "bold" }}>{t("chat:webFetch.wantsToAccess")}</span>
								</div>
								<div className="pl-6 text-vscode-descriptionForeground break-all">{urlText}</div>
							</>
						)
					}
					// 其他未知 tool 类型的兜底描述
						...
						<>
							<div style={headerStyle}>
								<span
									...
									style={{ color: "var(--vscode-foreground)", marginBottom: "-1.5px" }}
								/>
								<span style={{ fontWeight: "bold" }}>{t("chat:tool.wantsToPerformAction")}</span>
							</div>
							{urlText && (
								<div style={{ paddingTop: 10 }}>
									<Markdown markdown={urlText} partial={message.partial} />
								</div>
							)}
						</>
					)
