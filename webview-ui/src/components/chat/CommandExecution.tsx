...
export const CommandExecution = ({ executionId, text, icon, title }: CommandExecutionProps) => {
	...
	// 计算输出行数，超过阈值时默认折叠
	const OUTPUT_COLLAPSE_THRESHOLD = 3 // 超过3行默认折叠
	...
	// 当输出内容过长时，自动折叠（同时监听流式输出和已完成输出）
	const hasAutoCollapsedRef = useRef(false)
	useEffect(() => {
		const content = parsedOutput || streamingOutput
		if (!content || hasAutoCollapsedRef.current) return
		const lineCount = content.split("\n").length
		if (lineCount > OUTPUT_COLLAPSE_THRESHOLD) {
			setIsExpanded(false)
			hasAutoCollapsedRef.current = true
		}
	}, [parsedOutput, streamingOutput])
	...
	// 判断命令是否已完成：
	// 1. parsedOutput 存在说明消息 text 已包含输出（命令已完成）
	// 2. status 为 exited/timeout 说明收到了终止状态消息
	const isCommandCompleted = !!parsedOutput || status?.status === "exited" || status?.status === "timeout"
		...
		<>
			<div className="flex flex-row items-center justify-between gap-2 mb-1">
				<div className="flex flex-row items-center gap-2">
					...
					) : !status && text?.includes(COMMAND_OUTPUT_STRING) ? (
						<div className="flex flex-row items-center gap-2 font-mono text-xs">
							<StandardTooltip content={t("chat.commandExecution.exitStatus", { exitStatus: "?" })}>
								<div className="rounded-full size-2 bg-green-600 opacity-60" />
							</StandardTooltip>
						</div>
					) : null}
				</div>
				<div className=" flex flex-row items-center justify-between gap-2 px-1">
					<div className="flex flex-row items-center gap-1">
						{/* 只有当命令未完成且状态为 started 时才显示运行中状态 */}
						{!isCommandCompleted && status?.status === "started" && (
							<div className="flex flex-row items-center gap-2 font-mono text-xs">
