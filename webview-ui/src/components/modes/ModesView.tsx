...
const ModesView = () => {
	...
	// 并行子任务数量设置（通过消息通信，不依赖 ExtensionState 类型）
	const [maxParallelChildrenLocal, setMaxParallelChildrenLocal] = useState(10)
	const [parallelSaved, setParallelSaved] = useState(false)

	// 从后端读取已保存的并行子任务数量
	useEffect(() => {
		vscode.postMessage({ type: "getMaxParallelChildren" })
		const handler = (e: MessageEvent) => {
			if (e.data?.type === "maxParallelChildrenValue" && typeof e.data.value === "number") {
				setMaxParallelChildrenLocal(e.data.value)
			}
		}
				...
				<div className="mb-4">
					<div className="flex items-center justify-between mb-1">
						<label className="text-sm font-medium text-vscode-foreground">
							{t("prompts:modes.parallelTasks.label")}
						</label>
						<div className="flex items-center gap-2">
							{parallelSaved && <span className="text-xs text-vscode-charts-green">✓</span>}
							<span className="text-sm text-vscode-descriptionForeground">
								{maxParallelChildrenLocal}
							</span>
						</div>
					</div>
					<input
						...
						max={10}
						step={1}
						value={maxParallelChildrenLocal}
						onChange={(e) => {
							const val = parseInt(e.target.value, 10)
							setMaxParallelChildrenLocal(val)
							setParallelSaved(false)
						}}
						onMouseUp={() => {
							vscode.postMessage({
								type: "maxParallelChildren",
								value: maxParallelChildrenLocal,
							})
					...
					<div className="text-xs text-vscode-descriptionForeground mt-1">
						{t("prompts:modes.parallelTasks.description")}
					</div>
				</div>
			</Section>
								...
								span: (
									<span
										...
										onClick={() => {
											...
											vscode.postMessage({
												type: "openFile",
												text: `./.tocodex/rules-${currentMode.slug}/rules.md`,
												values: {
													create: true,
													content: "",
												},
