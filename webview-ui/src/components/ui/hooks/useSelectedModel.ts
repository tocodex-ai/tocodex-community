...
export const useSelectedModel = (apiConfiguration?: ProviderSettings) => {
	const provider = apiConfiguration?.apiProvider || "roo"
	...
	const { id, info } =
		apiConfiguration && isReady && activeProvider
			...
			: { id: getProviderDefaultModelId(activeProvider ?? "roo"), info: undefined }
...
function getSelectedModel({
	...
	switch (provider) {
		...
		case "roo": {
			// 默认选择模型列表的第1个，而非硬编码的 defaultModelId
			const rooModels = routerModels.roo
			const firstModelId = rooModels ? Object.keys(rooModels).sort()[0] : undefined
			const effectiveDefault = firstModelId ?? defaultModelId
			const id = getValidatedModelId(apiConfiguration.apiModelId, rooModels, effectiveDefault)
			const info = rooModels?.[id]
			// api/price 返回的模型可能缺少 supportsImages 字段，默认支持图片多模态
			return { id, info: info ? { ...info, supportsImages: info.supportsImages ?? true } : info }
		}
		case "qwen-code": {
			const id = apiConfiguration.apiModelId ?? defaultModelId
