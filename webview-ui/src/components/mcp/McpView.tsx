...
type McpViewProps = {
	showCodebaseIndexInChat?: boolean
	setShowCodebaseIndexInChat?: (value: boolean) => void
}

const McpView = ({ showCodebaseIndexInChat, setShowCodebaseIndexInChat }: McpViewProps = {}) => {
	const {
		mcpServers: servers,
		alwaysAllowMcp,
		mcpEnabled,
		showCodebaseIndexInChat: liveShowCodebaseIndexInChat,
	} = useExtensionState()
	const effectiveShowCodebaseIndexInChat = showCodebaseIndexInChat ?? liveShowCodebaseIndexInChat ?? false
	...
	return (
		<div>
			...
			<Section>
				...
				<CodebaseIndexChatToggle
					showCodebaseIndexInChat={effectiveShowCodebaseIndexInChat}
					setShowCodebaseIndexInChat={setShowCodebaseIndexInChat}
				/>
...
const CodebaseIndexChatToggle = ({
	showCodebaseIndexInChat,
	setShowCodebaseIndexInChat,
}: {
	showCodebaseIndexInChat: boolean
	setShowCodebaseIndexInChat?: (value: boolean) => void
}) => {
	...
	const handleChange = (e: Event | FormEvent<HTMLElement>) => {
		...
		if (setShowCodebaseIndexInChat) {
			setShowCodebaseIndexInChat(target.checked)
			return
		}

		vscode.postMessage({
			type: "updateSettings",
			updatedSettings: { showCodebaseIndexInChat: target.checked },
		})
	}

	return (
		<div style={{ marginBottom: "20px" }}>
			<VSCodeCheckbox checked={showCodebaseIndexInChat} onChange={handleChange}>
				<span style={{ fontWeight: "500" }}>{t("mcp:showCodebaseIndexInChat.title")}</span>
			</VSCodeCheckbox>
			<p
				style={{
					fontSize: "12px",
				...
				{t("mcp:showCodebaseIndexInChat.description")}
			</p>
		</div>
	)
}
