...
const StyledMarkdown = styled.div`
	* {
	...
	.inline-code-with-copy:hover .inline-code-copy-btn {
		opacity: 0.6;
	}
		...
		() => ({
			...
			a: ({ href, children, ...props }: any) => {
				const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
					...
					// Extract line number if present (avoid matching Windows drive letter "C:")
					...
					// Add ./ prefix if needed — but skip Windows absolute paths like C:\ or D:/
					const isWindowsAbsolute = /^[a-zA-Z]:[/\\]/.test(filePath)
					if (!filePath.startsWith("/") && !filePath.startsWith("./") && !isWindowsAbsolute) {
						filePath = "./" + filePath
					}

					// Image files: route to openImage (binary-safe), not openFile (which opens as text)
					const imageExtensions = /\.(png|jpe?g|gif|bmp|webp|svg|ico|tiff?)$/i
			...
			code: ({ children, className, ...props }: any) => {
				// This handles inline code — 带悬停复制按钮
				return (
