...
export function ImageViewer({
	...
	/**
	 * Saves the image as a file (Save As dialog)
	 */
	const handleSave = async (e: React.MouseEvent) => {
		e.stopPropagation()

		try {
			// Send the actual file path for saving (not the webview URI which is unresolvable by backend)
			vscode.postMessage({
				type: "saveImage",
				imagePath: imagePath, // actual file path for file-based copy
				dataUri: imageUri, // fallback for data URI based images (e.g. Mermaid diagrams)
			})
		} catch (error) {
	...
	const formatDisplayPath = (filePath: string): string => {
		...
		// If it's an absolute path, extract the relative portion after workspace root
		// Works for both /unix/path and C:/windows/path formats
