...
type AutoApproveSettingsProps = HTMLAttributes<HTMLDivElement> & {
	...
	alwaysAllowAllCommands?: boolean
	alwaysAllowFollowupQuestions?: boolean
	followupAutoApproveTimeoutMs?: number
	allowedCommands?: string[]
	allowedMaxRequests?: number | undefined
	allowedMaxCost?: number | undefined
	taskCostBudget?: number | null | undefined
		...
		<div {...props}>
			...
			<Section>
				<div className="space-y-4">
					...
					<SearchableSetting
						settingId="task-cost-budget"
						section="autoApprove"
						label={t("settings:autoApprove.taskCostBudget.title")}>
						<div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-2 items-center">
							<label htmlFor="task-cost-budget-input" className="text-sm whitespace-nowrap">
								{t("settings:autoApprove.taskCostBudget.title")}
							</label>
							<Input
								id="task-cost-budget-input"
								type="number"
								step="0.01"
								min="0"
								placeholder={t("settings:autoApprove.taskCostBudget.unlimited")}
								value={taskCostBudget != null ? taskCostBudget : ""}
								onChange={(e) => {
									const val = e.target.value
									if (val === "") {
										setCachedStateField("taskCostBudget", undefined)
									} else {
										const num = parseFloat(val)
										if (!isNaN(num) && num >= 0) {
											setCachedStateField("taskCostBudget", num)
										}
									}
								}}
								className="w-full"
								data-testid="task-cost-budget-input"
							/>
						</div>
						<div className="text-xs text-vscode-descriptionForeground mt-1">
							{t("settings:autoApprove.taskCostBudget.description")}
						</div>
					</SearchableSetting>
				</div>
