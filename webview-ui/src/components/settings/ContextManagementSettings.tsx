	...
	return (
		<div className={cn("flex flex-col gap-2", className)} {...props}>
			...
			<Section>
				...
				<SearchableSetting
					settingId="context-load-recent-plans"
					section="contextManagement"
					label={t("settings:contextManagement.loadRecentPlans.label")}>
					<VSCodeCheckbox
						checked={loadRecentPlans ?? true}
						onChange={(e: any) => setCachedStateField("loadRecentPlans", e.target.checked)}
						data-testid="load-recent-plans-checkbox">
						<label className="block font-medium mb-1">
							{t("settings:contextManagement.loadRecentPlans.label")}
						</label>
					</VSCodeCheckbox>
					<div className="text-vscode-descriptionForeground text-sm mt-1 mb-3">
						{t("settings:contextManagement.loadRecentPlans.description")}
					</div>
				</SearchableSetting>

				<SearchableSetting
					settingId="context-auto-generate-task-memo"
					section="contextManagement"
					label="自动生成任务备忘录">
					<VSCodeCheckbox
						checked={autoGenerateTaskMemo ?? true}
						onChange={(e: any) => setCachedStateField("autoGenerateTaskMemo", e.target.checked)}
						data-testid="auto-generate-task-memo-checkbox">
						<label className="block font-medium mb-1">
							自动生成任务备忘录
						</label>
					</VSCodeCheckbox>
					<div className="text-vscode-descriptionForeground text-sm mt-1 mb-3">
						任务完成时自动在 tocodex-docs/ 目录生成 Markdown 备忘录，记录任务描述、完成结果和变更文件。
					</div>
				</SearchableSetting>
