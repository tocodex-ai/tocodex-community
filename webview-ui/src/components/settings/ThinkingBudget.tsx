...
export const ThinkingBudget = ({ apiConfiguration, setApiConfigurationField, modelInfo }: ThinkingBudgetProps) => {
	...
	// Roo provider always shows effort select so users can manually set reasoning strength
	const isRooProvider = apiConfiguration.apiProvider === "roo"
	...
	const isReasoningEffortSupported = isRooProvider || (!!modelInfo && !!modelInfo.supportsReasoningEffort)
	...
	// 2. supportsReasoningEffort is boolean true (not an explicit array), OR it's a Roo provider
	...
	const shouldAutoAddDisable =
		...
		(supports === true || (isRooProvider && !Array.isArray(supports))) &&
		!baseAvailableOptions.includes("disable" as any)
