...
export const OpenAICompatible = ({
	...
	const onMessage = useCallback((event: MessageEvent) => {
		...
		switch (message.type) {
			case "openAiModels": {
				...
				// 优先读取带 isFree 字段的 openAiModelInfos（值通过 webviewMessageHandler 注入到 values 里）
				const infos = (
					message as unknown as { values?: { openAiModelInfos?: { id: string; isFree: boolean }[] } }
				...
				if (Array.isArray(infos) && infos.length > 0) {
					...
					// 兼容旧格式：只回 id 数组时按 :free 后缀推导
					setOpenAiModels(
						Object.fromEntries(
							updatedModels.map((id) => [
								id,
								{
									...openAiModelInfoSaneDefaults,
	...
	// 当 openAiBaseUrl 为空时，自动填入示例 URL；
	// 当 openAiStreamingEnabled 未设置时默认开启流式传输；
	// 当 openAiCustomModelInfo 未设置时使用 saneDefaults 但将 contextWindow 默认设为 200k。
	// 这些都是非用户操作（isUserAction=false），不会污染脏标记
	const DEFAULT_BASE_URL = "https://example.com/v1"
	const DEFAULT_CONTEXT_WINDOW = 200_000
	useEffect(() => {
		if (!apiConfiguration?.openAiBaseUrl) {
			setApiConfigurationField("openAiBaseUrl", DEFAULT_BASE_URL, false)
		}
		// 方案 1：每次进入 OpenAI Compatible 面板都强制默认勾选流式传输。
		// 原 `=== undefined` 检查无法处理历史 profile 中显式存为 false 的情况，
		// 这里改为：只要当前不是 true 就强制写回 true（非用户操作，不污染脏标记）。
		if (apiConfiguration?.openAiStreamingEnabled !== true) {
			setApiConfigurationField("openAiStreamingEnabled", true, false)
		}
		if (!apiConfiguration?.openAiCustomModelInfo) {
			setApiConfigurationField(
				"openAiCustomModelInfo",
				{ ...openAiModelInfoSaneDefaults, contextWindow: DEFAULT_CONTEXT_WINDOW },
				false,
			)
		}
	...
	return (
		<>
			<VSCodeTextField
				value={apiConfiguration?.openAiBaseUrl || DEFAULT_BASE_URL}
				type="url"
